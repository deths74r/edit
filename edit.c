...
static const char *help_text =
	...
	"\n"
	"COMMAND PROMPT\n"
	"  ESC                  Open command prompt (or clear selection)\n"
	"  Ctrl+Space           Open command prompt (always)\n"
	"  Tab                  Complete command name / cycle matches\n"
	"  Enter                Execute command (or search if no match)\n"
	"  ESC / Ctrl+C         Cancel prompt\n"
	"\n"
	"COMMANDS\n"
	"  save [file]          Save (optionally to a new file)\n"
	"  save as <file>       Save with a new filename\n"
	"  quit                 Quit (prompts if unsaved)\n"
	"  quit!                Quit immediately, no save check\n"
	"  undo                 Undo last change\n"
	"  redo                 Redo last undone change\n"
	"  find [text]          Search (pre-fills text if given)\n"
	"  replace              Find and replace\n"
	"  help                 This help screen\n"
	"  suspend              Suspend to shell (fg to resume)\n"
	"  theme [name]         Cycle theme or set by name\n"
	"  set tabstop <n>      Tab display width (1-32)\n"
	"  set ruler <n>        Column ruler (0 = off)\n"
	"  wrap                 Toggle word wrap\n"
	"  numbers              Toggle line numbers\n"
	"\n"
	"  Non-matching input becomes a search:\n"
	...
	"  Akira, Tokyo Cyberpunk, Clarity (colorblind-safe)\n"
	"\n"
	"SYNTAX HIGHLIGHTING\n"
	"  C/C++, Python, JavaScript, Go, Rust, Bash,\n"
	"  JSON, YAML, Markdown\n"
	"\n"
