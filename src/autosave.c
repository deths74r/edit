...
/* Autosave state */
static struct autosave_state autosave = {.enabled = true};
...
void autosave_set_enabled(bool enabled) { autosave.enabled = enabled; }

bool autosave_is_enabled(void) { return autosave.enabled; }

void autosave_set_swap_exists(bool exists) { autosave.swap_exists = exists; }
 ...
 */
static void autosave_generate_swap_path(const char *filename, char *swap_path,
                                        size_t size) {
...
void autosave_update_path(void) {
  autosave_generate_swap_path(editor.buffer.filename, autosave.swap_path,
...
int worker_process_autosave(struct task *task, struct task_result *result) {
  ...
  log_debug("Autosave complete: %zu bytes to %s", bytes_written,
            task->autosave.swap_path);
...
void autosave_handle_result(struct task_result *result) {
  ...
  if (result->error == -EEDIT_CANCELLED) {
    ...
    log_debug("Autosave successful: %zu bytes", result->autosave.bytes_written);
  }
}
 ...
 */
void autosave_mark_modified(void) { autosave.last_modify_time = time(NULL); }
...
void autosave_check(void) {
  ...
  uint32_t sample_count =
      editor.buffer.line_count < 1000 ? editor.buffer.line_count : 1000;
  ...
  /* Submit task */
  struct task task = {.type = TASK_AUTOSAVE, .task_id = task_generate_id()};
...
static void swap_dialog_draw_row(struct output_buffer *output,
  ...
  if (text != NULL) {
    ...
    for (int i = 0; i < text_len && chars_written < dialog->panel_width - 1;
         i++) {
...
static void swap_recovery_draw(struct output_buffer *output,
                               ...
                               const char *filename, const char *swap_path,
                               const char *time_str, size_t swap_size) {
  ...
  swap_dialog_draw_row(output, dialog, row++, "crashed or was interrupted.");
...
bool autosave_prompt_recovery(const char *filename, const char *swap_path) {
  ...
  swap_recovery_draw(&output, &dialog, filename, swap_path, time_str,
                     swap_size);
