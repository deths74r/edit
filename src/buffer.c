...
extern void syntax_highlight_line(struct line *line, struct buffer *buffer,
                                  uint32_t row);
...
int line_ensure_capacity_checked(struct line *line, uint32_t required) {
  ...
  uint32_t new_capacity =
      line->cell_capacity ? line->cell_capacity * 2 : INITIAL_LINE_CAPACITY;
  while (new_capacity < required)
    new_capacity *= 2;

  void *new_cells =
      edit_realloc(line->cells, new_capacity * sizeof(struct cell));
  if (IS_ERR(new_cells))
 ...
 * Returns 0 on success, -ENOMEM on allocation failure.
 */
int line_insert_cell_checked(struct line *line, uint32_t position,
                             uint32_t codepoint) {
 ...
 * Crashes on allocation failure.
 */
void line_insert_cell(struct line *line, uint32_t position,
                      uint32_t codepoint) {
 ...
 */
uint32_t cursor_prev_grapheme(struct line *line, struct buffer *buffer,
                              uint32_t column) {
  ...
  for (uint32_t i = start_column; i < column; i++) {
    int bytes = utf8_encode(line->cells[i].codepoint, utf8_buf + byte_len);
    byte_len += bytes;
  }

  /* Find previous grapheme boundary in UTF-8 */
  int prev_byte = utf8_prev_grapheme(utf8_buf, byte_len);
  ...
  while (offset < byte_len) {
    uint32_t cp;
    offset += utf8_decode(utf8_buf + offset, byte_len - offset, &cp);
    codepoints_after++;
  }
 ...
 * Handles emoji sequences, combining marks, flags, and Hangul.
 */
uint32_t cursor_next_grapheme(struct line *line, struct buffer *buffer,
                              uint32_t column) {
  line_warm(line, buffer);
  ...
  for (uint32_t i = column;
       i < line->cell_count && codepoints_encoded < GRAPHEME_LOOKAHEAD; i++) {
    int bytes = utf8_encode(line->cells[i].codepoint, utf8_buf + byte_len);
    byte_len += bytes;
    codepoints_encoded++;
  }
  ...
  int next_byte = utf8_next_grapheme(utf8_buf, byte_len, 0);
...
int buffer_ensure_capacity_checked(struct buffer *buffer, uint32_t required) {
  ...
  uint32_t new_capacity = buffer->line_capacity ? buffer->line_capacity * 2
                                                : INITIAL_BUFFER_CAPACITY;
  while (new_capacity < required)
    new_capacity *= 2;

  void *new_lines =
      edit_realloc(buffer->lines, new_capacity * sizeof(struct line));
  if (IS_ERR(new_lines))
...
void buffer_insert_cell_at_column(struct buffer *buffer, uint32_t row,
                                  uint32_t column, uint32_t codepoint) {
  int ret =
      buffer_insert_cell_at_column_checked(buffer, row, column, codepoint);
  BUG_ON(ret);
}
 ...
 * Crashes on allocation failure.
 */
void buffer_delete_grapheme_at_column(struct buffer *buffer, uint32_t row,
                                      uint32_t column) {
 ...
 * Returns 0 on success, -ENOMEM on failure.
 */
int buffer_insert_newline_checked(struct buffer *buffer, uint32_t row,
                                  uint32_t column) {
  if (row > buffer->line_count)
    return 0;
 ...
 * Crashes on allocation failure.
 */
void buffer_insert_newline(struct buffer *buffer, uint32_t row,
                           uint32_t column) {
 ...
 * Returns 0 on success, -ENOMEM on allocation failure.
 */
int buffer_load_from_memory(struct buffer *buffer, const char *content,
                            size_t size) {
  buffer_init(buffer);
  if (size == 0) {
  ...
  for (size_t i = 0; i <= size; i++) {
    ...
    if (is_end || is_newline) {
      ...
      while (offset < line_length) {
        uint32_t codepoint;
        int bytes =
            utf8_decode(line_start + offset, line_length - offset, &codepoint);
