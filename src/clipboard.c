...
extern int buffer_insert_newline_checked(struct buffer *buffer, uint32_t row,
                                         uint32_t column);
extern int buffer_insert_cell_at_column_checked(struct buffer *buffer,
                                                uint32_t row, uint32_t column,
                                                uint32_t codepoint);

/* Functions from undo.c */
extern void undo_begin_group(struct buffer *buffer, uint32_t row,
                             uint32_t column);
extern void undo_end_group(struct buffer *buffer, uint32_t row,
                           uint32_t column);
extern void undo_record_insert_char(struct buffer *buffer, uint32_t row,
                                    uint32_t column, uint32_t codepoint);
extern void undo_record_insert_newline(struct buffer *buffer, uint32_t row,
                                       uint32_t column);
...
static enum clipboard_tool clipboard_detect_tool(void) {
  ...
  /* Check for macOS clipboard tools */
...
char *clipboard_paste(size_t *out_length) {
  ...
  while (!feof(pipe)) {
    ...
    size_t read_count =
        fread(buffer + length, 1, CLIPBOARD_READ_CHUNK_SIZE, pipe);
    length += read_count;
...
void editor_paste(void) {
  ...
  if (ret) {
    editor_set_status_message("Paste failed after %u chars: %s", chars_inserted,
                              edit_strerror(ret));
  }
