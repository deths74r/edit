 ...
 */
static int __must_check line_warm_from_worker(struct line *line,
  ...
  while (byte_index < length) {
    ...
    uint32_t codepoint;
    int bytes = utf8_decode(text + byte_index, length - byte_index, &codepoint);
   ...
   * The release semantics ensure all writes are visible before temperature
   * change.
   */
  line->cells = cells;
...
int worker_process_warm_lines(struct task *task, struct task_result *result) {
  ...
  log_debug("Warmed %u lines, skipped %u (rows %u-%u)", warmed, skipped,
            start_row, end_row);
...
static int search_line_for_matches(struct line *line, struct buffer *buffer,
  ...
  if (temp == LINE_TEMPERATURE_COLD) {
    if (line_try_claim_warming(line)) {
      ...
      for (int i = 0;
           i < 100 && line_get_temperature(line) == LINE_TEMPERATURE_COLD;
           i++) {
        /* Spin */
      }
      if (line_get_temperature(line) == LINE_TEMPERATURE_COLD) {
  ...
  uint32_t *byte_to_cell =
      malloc((line->cell_count * 4 + 1) * sizeof(uint32_t));
  ...
  for (uint32_t col = 0; col < line->cell_count; col++) {
    char utf8[4];
    int bytes = utf8_encode(line->cells[col].codepoint, utf8);
    if (bytes > 0) {
  ...
  if (use_regex && compiled_regex != NULL) {
    ...
    while (offset < str_len) {
      ...
      uint32_t end_col =
          match_end < str_len ? byte_to_cell[match_end] : line->cell_count;

      /* Whole word check for regex */
      bool is_whole_word = true;
      if (whole_word) {
        if (match_start > 0 &&
            isalnum((unsigned char)line_str[match_start - 1])) {
          is_whole_word = false;
        }
        if (match_end < str_len &&
            isalnum((unsigned char)line_str[match_end])) {
          is_whole_word = false;
        }
      }
...
int worker_process_search(struct task *task, struct task_result *result) {
  ...
  for (uint32_t row = start_row; row < end_row; row++) {
    ...
    int matches = search_line_for_matches(line, &editor.buffer, row, pattern,
  ...
  log_debug("Search complete: %u matches in %u rows", total_matches,
            rows_searched);
 ...
 */
static char *expand_replacement_text(const char *replacement_str,
                                     const char *line_str, regmatch_t *matches,
                                     int nmatch) {
...
int worker_process_replace_all(struct task *task, struct task_result *result) {
  ...
  for (uint32_t row = 0; row < total_rows; row++) {
      ...
      if (line_try_claim_warming(line)) {
        if (line_get_temperature(line) == LINE_TEMPERATURE_COLD) {
          int __attribute__((unused)) err =
              line_warm_from_worker(line, &editor.buffer);
        }
        line_release_warming(line);
      }
    }
    ...
    if (use_regex && regex_valid) {
      ...
      while (offset < str_len) {
        ...
        char *expanded =
            expand_replacement_text(replacement_str, line_str, matches, 10);
        if (expanded) {
          replace_results_add(row, start_col, end_col, expanded,
                              strlen(expanded));
          free(expanded);
          total_replacements++;
        }
      ...
      if (pattern_len > 0) {
        ...
        while ((pos = strstr(pos, search_pattern)) != NULL) {
          ...
          if (is_whole_word) {
            ...
            replace_results_add(row, start_col, end_col, replacement_str,
                                strlen(replacement_str));
            total_replacements++;
          }
  ...
  log_debug("Replace search complete: %u replacements found",
            total_replacements);
...
void worker_process_results(void) {
  ...
  while (result_queue_pop(&result)) {
    ...
    if (result.error && result.error != -EEDIT_CANCELLED) {
      log_warn("Task %" PRIu64 " (type %d) failed: %s", result.task_id,
               result.type, edit_strerror(result.error));
    }
      ...
      if (result.error == -EEDIT_CANCELLED) {
        ...
        if (complete) {
          if (count == 0) {
            ...
            editor_set_status_message("Found %u match%s", count,
                                      count == 1 ? "" : "es");
...
static void editor_request_background_warming(void) {
  ...
  uint32_t warm_start =
      (viewport_start > lookahead) ? (viewport_start - lookahead) : 0;
  ...
  for (uint32_t row = warm_start; row < warm_end && !has_cold_lines; row++) {
    if (line_get_temperature(&editor.buffer.lines[row]) ==
        LINE_TEMPERATURE_COLD) {
      has_cold_lines = true;
    }
  }
  ...
  struct task task = {
      ...
      .warm = {.start_row = warm_start, .end_row = warm_end, .priority = 0}};
  ...
  if (err == 0) {
    ...
    log_debug("Queued warming task %lu for rows %u-%u", task.task_id,
              warm_start, warm_end);
  }
}
...
void emergency_save(void) {
  ...
  if (buffer->filename) {
    snprintf(emergency_path, sizeof(emergency_path), "%s.emergency.%d",
             buffer->filename, pid);
  } else {
    snprintf(emergency_path, sizeof(emergency_path), "/tmp/edit.emergency.%d",
             pid);
  }
  ...
  for (uint32_t row = 0; row < buffer->line_count; row++) {
    ...
    if (line_get_temperature(line) == LINE_TEMPERATURE_COLD) {
      ...
      if (buffer->mmap_base && line->mmap_length > 0) {
        fwrite(buffer->mmap_base + line->mmap_offset, 1, line->mmap_length,
               file);
      }
    } else {
...
char *stdin_read_all(size_t *out_size) {
  ...
  while (1) {
    ...
    ssize_t bytes_read =
        read(STDIN_FILENO, buffer + length, STDIN_READ_CHUNK_SIZE);
    if (bytes_read < 0) {
      if (errno == EINTR)
        continue;
      free(buffer);
...
static void multicursor_normalize(void) {
  ...
  /* Sort by position */
  qsort(editor.cursors, editor.cursor_count, sizeof(struct cursor),
        cursor_compare);

  /* Remove duplicates (cursors at same position) */
  uint32_t write_index = 1;
  for (uint32_t read_index = 1; read_index < editor.cursor_count;
       read_index++) {
 ...
 */
static bool multicursor_add(uint32_t row, uint32_t column, uint32_t anchor_row,
                            uint32_t anchor_column, bool has_selection) {
  if (editor.cursor_count == 0) {
    multicursor_enter();
  }
...
static int multicursor_cursor_at(uint32_t row, uint32_t column) {
  for (uint32_t i = 0; i < editor.cursor_count; i++) {
    if (editor.cursors[i].row == row && editor.cursors[i].column == column) {
      return (int)i;
    }
  }
  return -1;
 ...
 * Ensures wrap cache is computed first.
 */
static uint32_t line_get_segment_start(struct line *line, struct buffer *buffer,
                                       uint16_t segment) {
  line_ensure_wrap_cache(line, buffer);
 ...
 * Ensures wrap cache is computed first.
 */
static uint32_t line_get_segment_end(struct line *line, struct buffer *buffer,
                                     uint16_t segment) {
  line_ensure_wrap_cache(line, buffer);
 ...
 */
static int grapheme_display_width(struct line *line, uint32_t start,
                                  uint32_t end, uint32_t absolute_visual) {
  ...
  for (uint32_t i = start; i < end && i < line->cell_count; i++) {
    int w = utf8_cpwidth(line->cells[i].codepoint);
    if (w > 0) {
      return w;
    }
  }
...
static uint32_t line_find_column_at_visual(struct line *line,
  ...
  while (col < segment_end && col < line->cell_count) {
    ...
    int width =
        grapheme_display_width(line, col, grapheme_end, absolute_visual);
    visual_col += width;
    absolute_visual += width;
    col = grapheme_end;
  }
 ...
 */
static bool screen_row_to_line_segment(uint32_t screen_row, uint32_t *out_line,
  ...
  /* With wrap: iterate through lines and segments, starting from segment_offset
   */
  uint32_t file_row = editor.row_offset;
  uint16_t segment = editor.segment_offset;
  uint32_t screen_pos = 0;

  while (file_row < editor.buffer.line_count) {
    struct line *line = &editor.buffer.lines[file_row];
    line_ensure_wrap_cache(line, &editor.buffer);

    /* Check each segment of this line */
    while (segment < line->wrap_segment_count) {
...
/*
 * Scroll up by one screen row (one segment in wrap mode).
 * Returns true if scroll position changed.
 */
static bool scroll_up_one_row(void) {
  if (editor.wrap_mode == WRAP_NONE) {
  ...
  /* Wrap mode: scroll by segment */
  if (editor.segment_offset > 0) {
    editor.segment_offset--;
    return true;
  }

  /* At first segment - move to previous line's last segment */
  if (editor.row_offset > 0) {
    editor.row_offset--;
    struct line *line = &editor.buffer.lines[editor.row_offset];
    line_ensure_wrap_cache(line, &editor.buffer);
    editor.segment_offset = line->wrap_segment_count - 1;
    return true;
  }
...
/*
 * Scroll down by one screen row (one segment in wrap mode).
 * Returns true if scroll position changed.
 */
static bool scroll_down_one_row(void) {
  if (editor.wrap_mode == WRAP_NONE) {
  ...
  /* Wrap mode: scroll by segment */
  if (editor.row_offset >= editor.buffer.line_count) {
    return false;
  }
  ...
  if (editor.segment_offset + 1 < line->wrap_segment_count) {
    editor.segment_offset++;
    return true;
  }

  /* At last segment - move to next line's first segment */
  if (editor.row_offset + 1 < editor.buffer.line_count) {
    editor.row_offset++;
    editor.segment_offset = 0;
    return true;
  }
...
/*
 * Scroll up by n screen rows.
 */
static void scroll_up_rows(uint32_t count) {
  for (uint32_t i = 0; i < count; i++) {
    if (!scroll_up_one_row()) {
      break;
    }
  }
}
...
/*
 * Scroll down by n screen rows.
 */
static void scroll_down_rows(uint32_t count) {
  for (uint32_t i = 0; i < count; i++) {
    if (!scroll_down_one_row()) {
      break;
    }
  }
}
...
static void editor_scroll(void) {
  ...
  if (!editor.selection_active) {
    ...
    if (editor.cursor_row < editor.row_offset) {
      ...
      editor.segment_offset = 0;
    } else if (editor.cursor_row == editor.row_offset &&
               editor.wrap_mode != WRAP_NONE) {
      /* Cursor on first visible line - check segment */
      ...
      if (cursor_segment < editor.segment_offset) {
        editor.segment_offset = cursor_segment;
      }
    }
    ...
    if (editor.wrap_mode == WRAP_NONE) {
      ...
      for (uint32_t row = editor.row_offset;
           row <= editor.cursor_row && row < editor.buffer.line_count; row++) {
        struct line *line = &editor.buffer.lines[row];
        line_ensure_wrap_cache(line, &editor.buffer);
        if (row == editor.row_offset) {
          /* First visible line: subtract segment_offset */
          if (row == editor.cursor_row) {
            uint16_t cursor_segment = line_get_segment_for_column(
                line, &editor.buffer, editor.cursor_column);
            screen_row += cursor_segment - editor.segment_offset + 1;
          } else {
            screen_row += line->wrap_segment_count - editor.segment_offset;
          }
      ...
      /* If cursor is past visible area, scroll down by segments */
      while (screen_row > editor.screen_rows &&
             editor.row_offset < editor.buffer.line_count) {
        struct line *line = &editor.buffer.lines[editor.row_offset];
        line_ensure_wrap_cache(line, &editor.buffer);
        uint16_t remaining = line->wrap_segment_count - editor.segment_offset;
        if (remaining > 0) {
          screen_row--;
          editor.segment_offset++;
          if (editor.segment_offset >= line->wrap_segment_count) {
            editor.segment_offset = 0;
            editor.row_offset++;
          }
        } else {
  ...
  if (editor.wrap_mode == WRAP_NONE) {
    uint32_t render_column =
        editor_get_render_column(editor.cursor_row, editor.cursor_column);
...
static void editor_move_cursor(int key) {
  ...
  struct line *current_line = editor.cursor_row < editor.buffer.line_count
                                  ? &editor.buffer.lines[editor.cursor_row]
                                  : NULL;

  switch (base_key) {
  case KEY_ARROW_LEFT:
    if (editor.cursor_column > 0 && current_line) {
      editor.cursor_column = cursor_prev_grapheme(current_line, &editor.buffer,
  ...
  case KEY_ARROW_RIGHT:
    if (editor.cursor_column < line_length && current_line) {
      editor.cursor_column = cursor_next_grapheme(current_line, &editor.buffer,
                                                  editor.cursor_column);
  ...
  case KEY_ARROW_UP:
    if (editor.wrap_mode == WRAP_NONE) {
      ...
      uint32_t visual_col = line_get_visual_column_in_segment(
          current_line, &editor.buffer, cur_segment, editor.cursor_column);

      if (cur_segment > 0) {
        /* Move to previous segment of same line */
        uint32_t new_col = line_find_column_at_visual(
            current_line, &editor.buffer, cur_segment - 1, visual_col);
        ...
        uint32_t new_col = line_find_column_at_visual(prev_line, &editor.buffer,
                                                      last_segment, visual_col);
        editor.cursor_column = new_col;
      }
    }
    break;
  ...
  case KEY_ARROW_DOWN:
    if (editor.wrap_mode == WRAP_NONE) {
      ...
      if (cur_segment + 1 < current_line->wrap_segment_count) {
        ...
        uint32_t new_col = line_find_column_at_visual(next_line, &editor.buffer,
                                                      0, visual_col);
        editor.cursor_column = new_col;
      }
  ...
  case KEY_CTRL_ARROW_LEFT:
    if (current_line) {
      ...
      editor.cursor_column =
          find_prev_word_start(current_line, editor.cursor_column);
      /* If stuck at start of line, wrap to previous line */
      if (editor.cursor_column == 0 && old_col == 0 && editor.cursor_row > 0) {
        editor.cursor_row--;
  ...
  case KEY_CTRL_ARROW_RIGHT:
    if (current_line) {
      ...
      editor.cursor_column =
          find_next_word_start(current_line, editor.cursor_column);
  ...
  case KEY_HOME:
    if (editor.wrap_mode == WRAP_NONE || !current_line) {
      ...
      uint32_t segment_start =
          line_get_segment_start(current_line, &editor.buffer, segment);
      if (editor.cursor_column == segment_start && segment > 0) {
        /* Already at segment start, go to previous segment start */
        segment_start =
            line_get_segment_start(current_line, &editor.buffer, segment - 1);
      }
  ...
  case KEY_END:
    if (editor.wrap_mode == WRAP_NONE || !current_line) {
      ...
      uint32_t segment_end =
          line_get_segment_end(current_line, &editor.buffer, segment);
      if (editor.cursor_column == segment_end &&
        ...
        segment_end =
            line_get_segment_end(current_line, &editor.buffer, segment + 1);
      }
  ...
  if (editor.cursor_count > 0) {
    ...
    int32_t col_delta =
        (int32_t)editor.cursor_column - (int32_t)primary->column;
    for (uint32_t i = 0; i < editor.cursor_count; i++) {
      struct cursor *cursor = &editor.cursors[i];
...
void editor_delete_selection(void) {
  ...
  if (text != NULL) {
    undo_record_delete_text(&editor.buffer, start_row, start_col, end_row,
                            end_col, text, text_length);
    free(text);
  }
...
static bool editor_table_next_cell(void) {
  ...
  if (!table_detect_bounds(&editor.buffer, editor.cursor_row, &start_row,
  ...
  undo_record_insert_newline(&editor.buffer, editor.cursor_row,
                             editor.cursor_column);
...
static void editor_insert_character(uint32_t codepoint) {
  ...
  int ret = buffer_insert_cell_at_column_checked(
      &editor.buffer, editor.cursor_row, editor.cursor_column, codepoint);
  if (ret) {
...
static void editor_insert_newline(void) {
  ...
  if (syntax_is_markdown_file(editor.buffer.filename) &&
    ...
    if (md_is_table_separator(prev_line)) {
      uint32_t start_row, end_row, separator_row;
      if (table_detect_bounds(&editor.buffer, editor.cursor_row - 1, &start_row,
                              &end_row, &separator_row)) {
        ...
        if (col_count > 0) {
          ...
          for (uint32_t c = 0; c < col_count; c++) {
            undo_record_insert_char(&editor.buffer, editor.cursor_row, pos,
                                    ' ');
            buffer_insert_cell_at_column_checked(&editor.buffer,
  ...
  for (uint32_t i = 0; i < indent_count; i++) {
    ...
    ret = buffer_insert_cell_at_column_checked(
        &editor.buffer, editor.cursor_row, editor.cursor_column,
        indent_chars[i]);
    if (ret) {
  ...
  if (syntax_is_markdown_file(editor.buffer.filename) &&
    ...
    if (found_marker && marker_start != UINT32_MAX) {
      /* Check if there's any content after the marker (excluding trailing
       * whitespace) */
      ...
      if (!has_content) {
        ...
        for (uint32_t i = 0; i < delete_count; i++) {
          undo_record_delete_char(&editor.buffer, editor.cursor_row - 1,
                                  marker_start,
        ...
        if (first_cp == '-' || first_cp == '*' || first_cp == '+') {
          ...
          char num_str[16];
          int num_len =
              snprintf(num_str, sizeof(num_str), "%u%c ", num, delimiter);
        ...
        for (uint32_t i = 0; i < list_marker_count; i++) {
          undo_record_insert_char(&editor.buffer, editor.cursor_row,
                                  editor.cursor_column, list_marker[i]);
          buffer_insert_cell_at_column_checked(
              &editor.buffer, editor.cursor_row, editor.cursor_column,
              list_marker[i]);
          editor.cursor_column++;
        }
...
static void editor_delete_character(void) {
  ...
  if (editor.cursor_column < line->cell_count) {
    ...
    undo_record_delete_newline(&editor.buffer, editor.cursor_row,
                               editor.cursor_column);
  }

  int ret = buffer_delete_grapheme_at_column_checked(
      &editor.buffer, editor.cursor_row, editor.cursor_column);
  if (ret) {
    editor_set_status_message("Delete failed: %s", edit_strerror(ret));
    undo_end_group(&editor.buffer, editor.cursor_row, editor.cursor_column);
...
static void editor_handle_backspace(void) {
  ...
  if (editor.cursor_column > 0) {
    ...
    line_warm(line, &editor.buffer);
    uint32_t new_column =
        cursor_prev_grapheme(line, &editor.buffer, editor.cursor_column);
    /* Record the character we're about to delete */
    uint32_t codepoint = line->cells[new_column].codepoint;
    undo_record_delete_char(&editor.buffer, editor.cursor_row, new_column,
                            codepoint);
    editor.cursor_column = new_column;
    ret = buffer_delete_grapheme_at_column_checked(
        &editor.buffer, editor.cursor_row, editor.cursor_column);
    if (ret) {
      editor_set_status_message("Delete failed: %s", edit_strerror(ret));
      undo_end_group(&editor.buffer, editor.cursor_row, editor.cursor_column);
    ...
    uint32_t previous_line_length =
        editor_get_line_length(editor.cursor_row - 1);
    undo_record_delete_newline(&editor.buffer, editor.cursor_row - 1,
...
static void multicursor_insert_character(uint32_t codepoint) {
  ...
  for (int i = (int)editor.cursor_count - 1; i >= 0; i--) {
    ...
    /* Insert character */
    undo_record_insert_char(&editor.buffer, cursor->row, cursor->column,
                            codepoint);
    buffer_insert_cell_at_column(&editor.buffer, cursor->row, cursor->column,
                                 codepoint);
...
static void multicursor_backspace(void) {
  ...
  for (int i = (int)editor.cursor_count - 1; i >= 0; i--) {
    ...
    if (cursor->column > 0) {
      ...
      undo_record_delete_char(&editor.buffer, cursor->row, delete_column,
                              deleted_codepoint);
 ...
 */
static uint32_t screen_column_to_cell(uint32_t row,
                                      uint32_t target_visual_column) {
  if (row >= editor.buffer.line_count) {
    return 0;
  }
  ...
  while (cell_index < line->cell_count &&
         visual_column < target_visual_column) {
    uint32_t grapheme_end =
        cursor_next_grapheme(line, &editor.buffer, cell_index);
    int width =
        grapheme_display_width(line, cell_index, grapheme_end, visual_column);
...
static bool editor_select_word(uint32_t row, uint32_t column) {
  ...
  enum character_class click_class =
  ...
  while (word_start > 0) {
    enum character_class prev_class =
        neighbor_get_class(line->cells[word_start - 1].neighbor);
    if (!classes_form_word(prev_class, click_class) &&
        prev_class != click_class) {
      break;
    }
    word_start--;
  }
  ...
  while (word_end < line->cell_count - 1) {
    enum character_class next_class =
        neighbor_get_class(line->cells[word_end + 1].neighbor);
    if (!classes_form_word(click_class, next_class) &&
        next_class != click_class) {
      break;
    }
    word_end++;
  }
...
static bool search_matches_at(struct line *line, struct buffer *buffer,
                              uint32_t column, const char *query,
                              uint32_t query_len);
...
static void editor_select_next_occurrence(void) {
  ...
  if (find_next_occurrence(text, text_length, search_row, search_column,
                           true, /* wrap */
    ...
    if (is_original || already_exists) {
      ...
      editor_set_status_message("%u cursor%s (all occurrences)", count,
                                count > 1 ? "s" : "");
    } else {
      ...
      if (multicursor_add(found_row, new_cursor_column, found_row, found_column,
                          true)) {
        multicursor_normalize();

        editor_set_status_message("%u cursors", editor.cursor_count);
...
static uint32_t calculate_adaptive_scroll(int direction) {
  ...
  bool first_scroll =
      (last_scroll_time.tv_sec == 0 && last_scroll_time.tv_nsec == 0);

  /* Update timestamp */
  last_scroll_time = now;

  /* Reset velocity on direction change, first scroll, or timeout */
  if (direction != last_scroll_direction || dt > SCROLL_VELOCITY_TIMEOUT ||
      dt <= 0 || first_scroll) {
  ...
  return SCROLL_MIN_LINES +
         (uint32_t)(t * (SCROLL_MAX_LINES - SCROLL_MIN_LINES));
}
...
void editor_handle_mouse(struct mouse_input *mouse) {
  ...
  case MOUSE_LEFT_PRESS: {
    ...
    if (screen_row_to_line_segment(mouse->row, &file_row, &segment)) {
      ...
      struct line *line = &editor.buffer.lines[file_row];
      if (editor.wrap_mode != WRAP_NONE) {
        cell_col = line_find_column_at_visual(line, &editor.buffer, segment,
                                              screen_col);
      } else {
        cell_col =
            screen_column_to_cell(file_row, screen_col + editor.column_offset);
      }
    } else {
    ...
    if (now - last_click_time <= 1 && last_click_row == file_row &&
  ...
  case MOUSE_SCROLL_DOWN: {
    ...
    uint32_t scroll_amount = calculate_adaptive_scroll(1);
    scroll_down_rows(scroll_amount);
    break;
  }
...
/*
 * Enter search mode. Saves current cursor position and initializes search
 * state.
 */
...
static bool is_word_boundary(struct line *line, uint32_t column) {
  ...
  bool previous_is_word =
      isalnum(previous_codepoint) || previous_codepoint == '_';
...
static uint32_t search_match_length_at(struct line *line, uint32_t column) {
  ...
  if (search.use_regex) {
    ...
    uint32_t *byte_to_cell;
    char *line_string =
        line_to_string(line, column, &byte_to_cell, &string_length);
    ...
    while (query[query_position] != '\0' && cell_index < line->cell_count) {
      ...
      int bytes =
          utf8_decode(query + query_position,
                      search.query_length - query_position, &query_codepoint);
      if (bytes <= 0)
        break;
      ...
      if (search.case_sensitive) {
        ...
        if (lower_cell >= 'A' && lower_cell <= 'Z')
          lower_cell += 32;
        if (lower_query >= 'A' && lower_query <= 'Z')
          lower_query += 32;
...
static uint32_t search_query_cell_count(const char *query, uint32_t query_len) {
  ...
  while (offset < query_len) {
    uint32_t cp;
    int bytes = utf8_decode(query + offset, query_len - offset, &cp);
    if (bytes <= 0)
      break;
    count++;
    offset += bytes;
...
bool search_find_next(bool wrap) {
  ...
  for (uint32_t row = start_row; row < editor.buffer.line_count; row++) {
    ...
    for (uint32_t col = col_start; col < line->cell_count; col++) {
      if (search_matches_at(line, &editor.buffer, col, search.query,
                            search.query_length)) {
        editor.cursor_row = row;
...
bool search_find_previous(bool wrap) {
  ...
  for (int32_t row = start_row; row >= 0; row--) {
    ...
    int32_t col_start =
        (row == start_row) ? start_col : (int32_t)line->cell_count - 1;
    if (col_start < 0)
      continue;

    for (int32_t col = col_start; col >= 0; col--) {
      if (search_matches_at(line, &editor.buffer, (uint32_t)col, search.query,
                            search.query_length)) {
  ...
  if (wrap && editor.buffer.line_count > 0) {
    for (int32_t row = (int32_t)editor.buffer.line_count - 1; row >= start_row;
         row--) {
      struct line *line = &editor.buffer.lines[row];
      line_warm(line, &editor.buffer);
...
/*
 * Update search results when query changes. Finds first match from saved
 * position.
 */
static void search_update(void) {
  ...
  if (search_should_use_async()) {
    search_async_start(search.query, search.use_regex, search.case_sensitive,
                       search.whole_word);
...
static uint32_t replace_count_cells(const char *text, uint32_t length) {
  ...
  while (p < end) {
    uint32_t codepoint;
    int bytes = utf8_decode(p, end - p, &codepoint);
    if (bytes <= 0) {
      break;
    }
    p += bytes;
...
static char *expand_replacement(const char *replace_text,
                                const char *source_text, regmatch_t *matches,
                                size_t match_count) {
...
static bool search_replace_current(void) {
  ...
  if (search.use_regex && search.regex_compiled) {
    ...
    if (line_string) {
      ...
      if (regexec(&search.compiled_regex, line_string, 10, matches, 0) == 0) {
        final_replacement =
            expand_replacement(search.replace_text, line_string, matches, 10);
      }
      free(line_string);
      free(byte_to_cell);
    }
  ...
  while (*r) {
    uint32_t codepoint;
    int bytes = utf8_decode(r, strlen(r), &codepoint);
    if (bytes <= 0) {
      break;
    }
    r += bytes;

    undo_record_insert_char(&editor.buffer, editor.cursor_row, insert_position,
                            codepoint);
    ...
    if (insert_position < line->cell_count) {
      memmove(&line->cells[insert_position + 1], &line->cells[insert_position],
...
static void search_replace_and_next(void) {
  if (search_replace_current()) {
    ...
    uint32_t replace_cells =
        replace_count_cells(search.replace_text, search.replace_length);
...
static void search_replace_all(void) {
  ...
  search.has_match = false;
  editor_set_status_message("Replaced %u occurrence%s", count,
                            count != 1 ? "s" : "");
}
...
static int search_match_type(uint32_t row, uint32_t column) {
  ...
  if (search.has_match && row == search.match_row) {
    uint32_t current_match_len =
        search_match_length_at(line, search.match_column);
    if (current_match_len > 0 && column >= search.match_column &&
  ...
  uint32_t check_start =
      (column >= max_match_len) ? column - max_match_len + 1 : 0;
 ...
 *
 * Two rendering modes:
 * 1. Segment mode (end_cell < UINT32_MAX): Render cells from start_cell to
 * end_cell. Used for wrapped line segments.
 * 2. Scroll mode (end_cell == UINT32_MAX): Horizontal scroll - start_cell is a
 *    visual column offset, skip to that position then render. Used for
 * WRAP_NONE.
 *
 * Parameters:
 ...
 *   start_cell    - First cell index, OR visual column offset if
 * end_cell==UINT32_MAX end_cell      - Last cell index (exclusive), or
 * UINT32_MAX for scroll mode max_width     - Maximum visual width to render
...
static void render_line_content(struct output_buffer *output, struct line *line,
  ...
  if (end_cell == UINT32_MAX) {
    ...
    while (cell_index < line->cell_count &&
           visual_column < (int)column_offset) {
  ...
  int current_highlight =
      0; /* 0=normal, 1=selected, 2=search_other, 3=search_current */
  ...
  /* Set initial style with cursor line background if applicable */
  {
    ...
    struct syntax_color bg =
        is_cursor_line ? active_theme.cursor_line : style->bg;
    int length = snprintf(
        escape, sizeof(escape), "\x1b[0;48;2;%d;%d;%d;38;2;%d;%d;%dm", bg.red,
        bg.green, bg.blue, style->fg.red, style->fg.green, style->fg.blue);
    length +=
        attr_to_escape(style->attr, escape + length, sizeof(escape) - length);
  ...
  if (editor.hybrid_mode && is_cursor_line &&
    ...
    md_should_reveal_element(line, editor.cursor_column, &reveal_start,
                             &reveal_end);
  }
  /*
  ...
  if (editor.hybrid_mode && line->cell_count > 0 &&
    ...
    struct syntax_color bg =
        is_cursor_line ? active_theme.cursor_line : style->bg;
    char escape[128];
    int esc_len = snprintf(
        escape, sizeof(escape), "\x1b[0;48;2;%d;%d;%d;38;2;%d;%d;%dm", bg.red,
        bg.green, bg.blue, style->fg.red, style->fg.green, style->fg.blue);
    esc_len +=
        attr_to_escape(style->attr, escape + esc_len, sizeof(escape) - esc_len);
  ...
  while (cell_index < end_cell && rendered_width < max_width) {
    ...
    if (editor.hybrid_mode && (flags & CELL_FLAG_HIDEABLE) &&
        syntax_is_markdown_file(buffer->filename) && !cursor_in_code_block) {
      /* Check if this cell is in the reveal range */
      bool in_reveal_range =
          (cell_index >= reveal_start && cell_index < reveal_end);
      if (!in_reveal_range) {
     ...
     * secondary cursor > search current > search other > selection > trailing
     * ws > cursor line
     */
    int highlight = 0;
    ...
    if (syntax != current_syntax || highlight != current_highlight) {
      ...
      int length = snprintf(escape, sizeof(escape),
                            "\x1b[0;48;2;%d;%d;%d;38;2;%d;%d;%dm", bg.red,
                            bg.green, bg.blue, fg.red, fg.green, fg.blue);
    ...
    if (codepoint == '\t') {
      ...
      if (editor.show_whitespace) {
        ...
        int ws_len = style_to_escape_with_bg(
            &active_theme.whitespace_tab, ws_bg, ws_escape, sizeof(ws_escape));
        ...
        struct syntax_color restore_bg =
            is_cursor_line ? active_theme.cursor_line : restore_style->bg;
        char restore_escape[128];
        int restore_len =
            snprintf(restore_escape, sizeof(restore_escape),
                     "\x1b[0;48;2;%d;%d;%d;38;2;%d;%d;%dm", restore_bg.red,
                     restore_bg.green, restore_bg.blue, restore_style->fg.red,
                     restore_style->fg.green, restore_style->fg.blue);
        restore_len +=
            attr_to_escape(restore_style->attr, restore_escape + restore_len,
      ...
      if (rendered_width + width <= max_width) {
        ...
        for (uint32_t gi = cell_index;
             gi < grapheme_end && gi < line->cell_count; gi++) {
          char utf8_buffer[UTF8_MAX_BYTES];
          ...
          int bytes = utf8_encode(output_codepoint, utf8_buffer);
    ...
    if (first_syntax == SYNTAX_MD_CODE_BLOCK ||
      ...
      if (remaining > 0) {
        ...
        struct syntax_color bg =
            is_cursor_line ? active_theme.cursor_line : style->bg;
        char escape[64];
        int esc_len = snprintf(escape, sizeof(escape), "\x1b[0;48;2;%d;%d;%dm",
   ...
   */
  struct syntax_color fill_bg =
      is_cursor_line ? active_theme.cursor_line : active_theme.background;
  char bg_escape[48];
  snprintf(bg_escape, sizeof(bg_escape), "\x1b[48;2;%d;%d;%dm", fill_bg.red,
           fill_bg.green, fill_bg.blue);
 ...
 */
static void ensure_line_warm_for_render(struct line *line,
                                        struct buffer *buffer, uint32_t row) {
  int temp = line_get_temperature(line);
  ...
  if (temp == LINE_TEMPERATURE_COLD) {
    ...
    if (line_try_claim_warming(line)) {
      ...
      for (int i = 0;
           i < 1000 && line_get_temperature(line) == LINE_TEMPERATURE_COLD;
           i++) {
        /* Brief spin */
      }
...
static void render_draw_rows(struct output_buffer *output) {
  ...
  if (editor.cursor_row < editor.buffer.line_count) {
    ...
    cursor_segment = line_get_segment_for_column(cursor_line, &editor.buffer,
                                                 editor.cursor_column);
  }
  ...
  for (uint32_t screen_row = 0; screen_row < editor.screen_rows; screen_row++) {
    ...
    if (file_row >= editor.buffer.line_count && !is_empty_buffer_first_line) {
      ...
      if (editor.show_line_numbers && editor.gutter_width > 0) {
        if (segment == 0) {
          ...
          struct style *ln_style = is_cursor_line_segment
                                       ? &active_theme.line_number_active
                                       : &active_theme.line_number;
          struct syntax_color ln_bg = is_cursor_line_segment
                                          ? active_theme.gutter_active.bg
                                          : active_theme.gutter.bg;

          char color_escape[128];
          int escape_len = style_to_escape_with_bg(
              ln_style, ln_bg, color_escape, sizeof(color_escape));
          ...
          struct syntax_color wrap_bg = is_cursor_line_segment
                                            ? active_theme.gutter_active.bg
                                            : active_theme.wrap_indicator.bg;

          char color_escape[128];
          int escape_len =
              style_to_escape_with_bg(&active_theme.wrap_indicator, wrap_bg,
      ...
      uint32_t start_cell =
          line_get_segment_start(line, &editor.buffer, segment);
      ...
      if (editor.wrap_mode == WRAP_NONE) {
        ...
        render_line_content(output, line, &editor.buffer, file_row,
                            editor.column_offset, UINT32_MAX, text_area_width,
                            is_cursor_line_segment);
      } else {
        /* Wrap enabled: render this segment */
        render_line_content(output, line, &editor.buffer, file_row, start_cell,
                            end_cell, text_area_width, is_cursor_line_segment);
      }
      ...
      if (is_cursor_line_segment) {
        if (editor.color_column > 0) {
          ...
          while (idx < line->cell_count) {
            uint32_t grapheme_end =
                cursor_next_grapheme(line, &editor.buffer, idx);
            int w = grapheme_display_width(line, idx, grapheme_end,
                                           line_visual_width);
            line_visual_width += w;
            idx = grapheme_end;
          }
          ...
          if (col_pos >= line_visual_width &&
            ...
            char cursor_bg[48];
            snprintf(cursor_bg, sizeof(cursor_bg), "\x1b[48;2;%d;%d;%dm",
            ...
            snprintf(fill_escape, sizeof(fill_escape),
                     "\x1b[48;2;%d;%d;%dm\x1b[K", active_theme.cursor_line.red,
                     active_theme.cursor_line.green,
                     active_theme.cursor_line.blue);
        ...
        char reset_bg[48];
        snprintf(reset_bg, sizeof(reset_bg), "\x1b[48;2;%d;%d;%dm",
                 active_theme.background.red, active_theme.background.green,
                 active_theme.background.blue);
        ...
        char fill_escape[64];
        snprintf(fill_escape, sizeof(fill_escape), "\x1b[48;2;%d;%d;%dm\x1b[K",
                 active_theme.background.red, active_theme.background.green,
                 active_theme.background.blue);
        output_buffer_append_string(output, fill_escape);
      }
...
static void render_draw_status_bar(struct output_buffer *output) {
  ...
  snprintf(color_escape, sizeof(color_escape), "\x1b[0m\x1b[48;2;%u;%u;%um",
  ...
  const char *filename =
      editor.buffer.filename ? editor.buffer.filename : "[No Name]";
  char filename_buf[104];
  int filename_len =
      snprintf(filename_buf, sizeof(filename_buf), " %.100s", filename);
  ...
  if (editor.link_preview_active && editor.link_url_preview[0] != '\0') {
    ...
    if ((int)url_len > max_url_len) {
      ...
      right_length = snprintf(right_status, sizeof(right_status), "%.200s... ",
                              editor.link_url_preview);
      right_status[max_url_len] = '\0';
      right_length = (int)strlen(right_status);
    } else {
      right_length = snprintf(right_status, sizeof(right_status), "%s ",
                              editor.link_url_preview);
    }
  } else {
  ...
  escape_len =
      style_to_escape(&active_theme.status, color_escape, sizeof(color_escape));
  output_buffer_append(output, color_escape, escape_len);
...
static void render_draw_message_bar(struct output_buffer *output) {
  ...
  if (search.active) {
    bool has_regex_error =
        search.use_regex && !search.regex_compiled && search.query_length > 0;

    /* Label: "Search" or "Find" */
    message_bar_set_style(output, &active_theme.prompt_label);
    output_buffer_append_string(output,
                                search.replace_mode ? "Find" : "Search");
  ...
  if (message_length > 0 &&
      time(NULL) - editor.status_message_time < STATUS_MESSAGE_TIMEOUT) {
...
int __must_check render_refresh_screen(void) {
  ...
  if (editor.wrap_mode == WRAP_NONE) {
     ...
     * Account for segment_offset on the first visible line.
     */
    ...
    for (uint32_t row = editor.row_offset;
      ...
      line_ensure_wrap_cache(line, &editor.buffer);
      if (row == editor.row_offset) {
        /* First line: subtract segment_offset */
        cursor_screen_row += line->wrap_segment_count - editor.segment_offset;
      } else {
    ...
    if (editor.cursor_row < editor.buffer.line_count) {
      ...
      if (editor.cursor_row == editor.row_offset) {
        /* Cursor on first visible line: subtract segment_offset */
        cursor_screen_row += cursor_segment - editor.segment_offset;
      } else {
      ...
      uint32_t visual_col = line_get_visual_column_in_segment(
          cursor_line, &editor.buffer, cursor_segment, editor.cursor_column);
...
static bool editor_open_file(const char *path) {
  ...
  editor_set_status_message("Opened: %s (%u lines)", path,
...
/*
 * Handle Alt+T to cycle through themes.
 * Shows preview in status bar, Enter confirms, other keys cancel.
 */
static void editor_cycle_theme(void) {
  /* Start preview mode if not already active */
  if (!editor.theme_preview_active) {
    editor.theme_preview_active = true;
    editor.theme_preview_original = current_theme_index;
  }

  /* Cycle to next theme */
  int next = (current_theme_index + 1) % theme_count;
  theme_apply_by_index(next);

  /* Show theme name in status bar */
  editor_set_status_message("Theme: %s (Enter to confirm, Esc to cancel)",
                            active_theme.name);
}

/*
 * Confirm the currently previewed theme.
 */
static void editor_confirm_theme(void) {
  if (editor.theme_preview_active) {
    editor.theme_preview_active = false;
    config_save();
    editor_set_status_message("Theme set to %s", active_theme.name);
  }
}

/*
 * Cancel theme preview and restore original.
 */
static void editor_cancel_theme_preview(void) {
  if (editor.theme_preview_active) {
    theme_apply_by_index(editor.theme_preview_original);
    editor.theme_preview_active = false;
    editor_set_status_message("Theme preview cancelled");
  }
}
...
static bool search_handle_key(int key) {
  ...
  case '\x1b':         /* Escape - cancel search */
    ...
    editor_set_status_message(search.replace_mode ? "Replace cancelled"
                                                  : "Search cancelled");
    return true;
  ...
  case KEY_ALT_C:
    ...
    search_update();
    editor_set_status_message("Case %s", search.case_sensitive ? "sensitive"
                                                               : "insensitive");
    return true;
  ...
  default:
    ...
    if ((key >= 32 && key < 127) || key >= 128) {
      char utf8[4];
      int bytes = utf8_encode((uint32_t)key, utf8);

      if (search.replace_mode && search.editing_replace) {
        /* Insert into replace field */
        if (bytes > 0 && search.replace_length + (uint32_t)bytes <
                             sizeof(search.replace_text) - 1) {
        ...
        if (bytes > 0 &&
            search.query_length + (uint32_t)bytes < sizeof(search.query) - 1) {
...
static void editor_delete_line(void) {
  ...
  if (text != NULL) {
    for (uint32_t i = 0; i < line->cell_count; i++) {
      char utf8[4];
      int bytes = utf8_encode(line->cells[i].codepoint, utf8);
      if (bytes > 0) {
    ...
    uint32_t end_col =
        (row < editor.buffer.line_count - 1) ? 0 : line->cell_count;
    undo_record_delete_text(&editor.buffer, row, 0, end_row, end_col, text,
                            text_len);
    free(text);
  }
...
static void editor_duplicate_line(void) {
  ...
  undo_record_insert_newline(&editor.buffer, editor.cursor_row,
                             editor.cursor_column);
  buffer_insert_newline(&editor.buffer, editor.cursor_row,
                        editor.cursor_column);
...
static uint32_t line_comment_start(struct line *line) {
  ...
  /* Check for // */
  if (column + 1 < line->cell_count && line->cells[column].codepoint == '/' &&
...
static void editor_jump_to_match(void) {
  ...
  if (search_column < line->cell_count &&
    ...
    found = buffer_find_pair_partner(&editor.buffer, editor.cursor_row,
                                     search_column, &match_row, &match_column);
  }

  /* If not found at cursor, scan forward on the line */
  if (!found) {
    for (uint32_t column = search_column + 1; column < line->cell_count;
         column++) {
      if (is_matchable_bracket(line->cells[column].codepoint)) {
        found = buffer_find_pair_partner(&editor.buffer, editor.cursor_row,
                                         column, &match_row, &match_column);
        if (found) {
          break;
        }
      }
...
static void editor_toggle_comment(void) {
  ...
  for (uint32_t row = start_row; row <= end_row; row++) {
    ...
    if (should_comment) {
      ...
      memmove(&line->cells[insert_position + 3], &line->cells[insert_position],
              (line->cell_count - insert_position) * sizeof(struct cell));
      ...
      for (uint32_t i = chars_to_remove; i > 0; i--) {
        ...
        uint32_t codepoint = line->cells[delete_position].codepoint;
        undo_record_delete_char(&editor.buffer, row, delete_position,
                                codepoint);
      }
      ...
      memmove(&line->cells[comment_start],
              ...
              (line->cell_count - comment_start - chars_to_remove) *
                  sizeof(struct cell));
  ...
  editor_set_status_message("%s %u line%s",
                            should_comment ? "Commented" : "Uncommented", count,
                            count > 1 ? "s" : "");
}
 ...
 */
bool execute_action(enum editor_action action) {
  switch (action) {
  case ACTION_NONE:
    return false;
  ...
  case ACTION_CYCLE_COLOR_COLUMN:
    ...
    if (editor.color_column > 0) {
      editor_set_status_message(
          "Column %u (%s)", editor.color_column,
  ...
  case ACTION_TOGGLE_HYBRID_MODE:
    if (syntax_is_markdown_file(editor.buffer.filename)) {
      ...
      editor_set_status_message(
          "Hybrid mode only available for Markdown files");
    }
    return true;

  case ACTION_TOGGLE_BAR_POSITION:
    editor.bar_at_top = !editor.bar_at_top;
    editor_set_status_message("Status bar at %s",
  ...
  case ACTION_FORMAT_TABLES:
    if (syntax_is_markdown_file(editor.buffer.filename)) {
      ...
      editor_set_status_message(
          "Table formatting only available in markdown files");
    }
    return true;
...
void editor_process_keypress(void) {
  ...
  if (editor.theme_preview_active) {
    if (key == '\r' || key == '\n') {
      editor_confirm_theme();
      return;
    }
    if (key == '\x1b') {
      editor_cancel_theme_preview();
      return;
    }
    /* Alt+T continues cycling, other keys cancel preview */
    enum editor_action preview_action = keybinding_lookup(key);
    if (preview_action != ACTION_CYCLE_THEME) {
      editor_cancel_theme_preview();
