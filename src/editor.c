...
void editor_set_status_message(const char *format, ...) {
  ...
  vsnprintf(editor.status_message, sizeof(editor.status_message), format,
            arguments);
  va_end(arguments);
...
const char *color_column_char(enum color_column_style style) {
  ...
  case COLOR_COLUMN_DOTTED:
    return "\xe2\x94\x8a"; /* U+250A */
  case COLOR_COLUMN_HEAVY:
    return "\xe2\x94\x83"; /* U+2503 */
  default:
    return NULL;
  }
}
...
void editor_cycle_color_column_style(void) {
  ...
  editor.color_column_style =
      (editor.color_column_style + 1) % (COLOR_COLUMN_HEAVY + 1);

  const char *indicator = color_column_char(editor.color_column_style);
  if (indicator) {
    editor_set_status_message(
        "Column %u style: %s", editor.color_column,
 ...
 */
void selection_clear(void) { editor.selection_active = false; }
 ...
 */
bool editor_has_multi_cursor(void) { return editor.cursor_count > 1; }
...
bool goto_handle_key(int key) {
  ...
  if (key >= '0' && key <= '9' &&
      goto_line.input_length < sizeof(goto_line.input) - 1) {
 ...
 */
bool goto_line_is_active(void) { return goto_line.active; }

/*
 * Get the current go-to-line input string.
 */
const char *goto_line_get_input(void) { return goto_line.input; }
...
bool save_as_handle_key(int key) {
  ...
  if (key >= 32 && key < 127 &&
      save_as.path_length < sizeof(save_as.path) - 1) {
 ...
 */
bool save_as_is_active(void) { return save_as.active; }

/*
 * Check if save-as is prompting for overwrite confirmation.
 */
bool save_as_is_confirm_overwrite(void) { return save_as.confirm_overwrite; }

/*
 * Get the current path in save-as dialog.
 */
const char *save_as_get_path(void) { return save_as.path; }
...
bool quit_prompt_handle_key(int key) {
  ...
  if (key == 'y' || key == 'Y') {
    ...
    if (editor.buffer.filename == NULL) {
      editor_set_status_message(
          "No filename. Use Alt+Shift+S to Save As, then quit.");
      return true;
    }
    editor_save();
    if (!editor.buffer.is_modified) {
 ...
 */
bool reload_prompt_is_active(void) { return reload_prompt.active; }
...
void editor_check_for_updates(void) {
  ...
  if (editor.update_available) {
    editor_set_status_message(
        "Update v%s available (current: v%s). Install? [y/n]: ",
