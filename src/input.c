...
/* (Removed: Kitty keyboard protocol key state tracking - no longer needed) */

/*****************************************************************************
 * Dialog Mouse Mode
 *****************************************************************************/

void input_set_dialog_mouse_mode(bool enabled) { dialog_mouse_mode = enabled; }

bool input_get_dialog_mouse_mode(void) { return dialog_mouse_mode; }

struct mouse_input input_get_last_mouse(void) { return dialog_last_mouse; }

void input_clear_last_mouse(void) { dialog_last_mouse.event = MOUSE_NONE; }
...
bool input_is_mouse_event(int key) { return key == KEY_MOUSE_EVENT; }
...
/*
 * Read a key with timeout. Returns:
 *   > 0: key code
...
/*
 * Convert Kitty keycode + modifiers to internal key enum.
 */
static int map_csi_u_codepoint(int codepoint, int modifiers) {
  bool shift = (modifiers & KITTY_MOD_SHIFT) != 0;
  bool alt = (modifiers & KITTY_MOD_ALT) != 0;
  bool ctrl = (modifiers & KITTY_MOD_CTRL) != 0;
  (void)ctrl;

  /* Handle special keys (Kitty functional key codes) */
  switch (codepoint) {
  case 57419:
    return KEY_ARROW_UP;
  case 57420:
  ...
  case 57417:
    return alt ? KEY_ALT_ARROW_LEFT : KEY_ARROW_LEFT;
  case 57418:
    return alt ? KEY_ALT_ARROW_RIGHT : KEY_ARROW_RIGHT;
  case 57423:
    return KEY_HOME;
  case 57424:
    return KEY_END;
  ...
  /* ASCII letters with Alt modifier */
  if (alt && codepoint >= 'a' && codepoint <= 'z') {
    switch (codepoint) {
    case 'q':
      return KEY_ALT_Q;
    case 's':
      return shift ? KEY_ALT_SHIFT_S : KEY_ALT_S;
    case 'z':
      return shift ? KEY_ALT_SHIFT_Z : KEY_ALT_Z;
    case 'x':
      return KEY_ALT_X;
    case 'c':
      return shift ? KEY_ALT_SHIFT_C : KEY_ALT_C;
    case 'v':
      return KEY_ALT_V;
    case 'k':
      return KEY_ALT_K;
    ...
    case 'l':
      return KEY_ALT_L;
    case 'w':
      return shift ? KEY_ALT_SHIFT_W : KEY_ALT_W;
    case 'r':
      return KEY_ALT_R;
    case 'u':
      return KEY_ALT_U;
 ...
 * buffer contains "number" or "number;modifier"
 * terminator is the final character (u, A, B, C, D, H, F, ~, etc.)
 */
static int map_extended_csi(const char *buffer, char terminator) {
  int num1 = 0, modifier = 1;
  int event = KITTY_KEY_EVENT_PRESS; /* Default to press event */
  ...
  if (terminator == 'u') {
    ...
    return map_csi_u_codepoint(num1, mod_bits);
  }
  ...
  if (*p == ';') {
    ...
    /* Parse optional event type: modifier:event */
    if (*p == ':') {
      int parsed_event = 0;
  ...
  /* Filter release events for all non-CSI-u sequences */
 ...
 */
static int parse_extended_csi(char first_digit) {
  char c;
  char buffer[32];
  int buf_idx = 0;
 ...
 * - KEY_MOUSE_EVENT (-3): mouse event occurred, retrieve via
 * input_get_last_mouse()
...
int input_read_key(void) {
  ...
  if (character == '\x1b') {
    ...
    /* Only handle CSI and SS3 sequences */
    ...
    if (sequence[0] == '[') {
      if (sequence[1] >= '0' && sequence[1] <= '9') {
        /* Use extended CSI parser for Kitty keyboard protocol and legacy */
        int key = parse_extended_csi(sequence[1]);
        if (key == -2) {
          /* Key release event - ignore and read next key */
          return input_read_key();
        }
        if (key != -1) {
          return key;
      ...
      case 'F':
        return KEY_END;
      case 'P':
        return KEY_F1; /* F1 in xterm */
      case 'R':
        return KEY_F3; /* F3 in xterm */
      }
    }
  ...
  if (character & 0x80) {
    ...
    uint32_t codepoint;
    utf8_decode(utf8_buffer, bytes_to_read + 1, &codepoint);
