...
static const struct {
  ...
  const char *name;
                    ...
                    {ACTION_HELP, "help"},
                    {ACTION_CYCLE_THEME, "cycle_theme"},
...
void keybinding_load_defaults(void) {
  ...
  /* File operations */
  keybinding_add(KEY_ALT_Q, ACTION_QUIT);
  keybinding_add(KEY_ALT_S, ACTION_SAVE);
  keybinding_add(KEY_ALT_SHIFT_S, ACTION_SAVE_AS);
  /* Open file removed - use command line */
  /* New file - no shortcut (use menu or command) */

  /* Edit operations */
  keybinding_add(KEY_ALT_Z, ACTION_UNDO);
  keybinding_add(KEY_ALT_SHIFT_Z, ACTION_REDO);
  keybinding_add(KEY_ALT_X, ACTION_CUT);
  keybinding_add(KEY_ALT_C, ACTION_COPY);
  keybinding_add(KEY_ALT_V, ACTION_PASTE);
  ...
  keybinding_add(KEY_ALT_G, ACTION_GO_TO_LINE);
  ...
  keybinding_add(KEY_ALT_A, ACTION_SELECT_ALL);
  keybinding_add(KEY_ALT_W, ACTION_SELECT_WORD);
  /* Alt+D is duplicate line - multi-cursor via selection */

  /* Search */
  keybinding_add(KEY_ALT_F, ACTION_FIND);

  keybinding_add(KEY_ALT_N, ACTION_FIND_NEXT);
  keybinding_add(KEY_ALT_P, ACTION_FIND_PREV);
  ...
  keybinding_add(KEY_ALT_T, ACTION_CYCLE_THEME);
...
static void keybinding_apply_macos_overrides(void) {
...
int keybinding_parse_key(const char *str) {
  ...
  if (strcasecmp(str, "PageUp") == 0 || strcasecmp(str, "PgUp") == 0) {
    if (shift)
      return KEY_SHIFT_PAGE_UP;
    return KEY_PAGE_UP;
  }
  if (strcasecmp(str, "PageDown") == 0 || strcasecmp(str, "PgDn") == 0) {
    if (shift)
      return KEY_SHIFT_PAGE_DOWN;
  ...
  if (str[0] == 'F' || str[0] == 'f') {
    ...
    if (num == 1)
      return KEY_F1;
    if (num == 3)
      return shift ? KEY_SHIFT_F3 : KEY_F3;
  ...
  if (strlen(str) == 1 && isalpha((unsigned char)str[0])) {
    ...
    if (alt) {
      ...
      case 'w':
        return KEY_ALT_W;
      case 'z':
        return shift ? KEY_ALT_SHIFT_Z : KEY_ALT_Z;
      case 's':
        return shift ? KEY_ALT_SHIFT_S : 0;
  ...
  if (strcmp(str, "/") == 0 || strcmp(str, "Slash") == 0) {
    if (alt)
      return KEY_ALT_SLASH;
    if (ctrl)
      return 0x1f; /* Ctrl+/ */
  }
  if (strcmp(str, "]") == 0 || strcmp(str, "RightBracket") == 0) {
    if (alt)
      return KEY_ALT_BRACKET;
    if (ctrl)
      return 0x1d; /* Ctrl+] */
  }
...
const char *keybinding_key_string(enum editor_action action, char *buffer,
                                  size_t size) {
  ...
  for (int i = 0; i < binding_count; i++) {
    ...
    switch (key) {
    case KEY_F1:
      snprintf(buffer, size, "F1");
      return buffer;
    case KEY_F3:
      snprintf(buffer, size, "F3");
      return buffer;
    case KEY_SHIFT_F3:
      snprintf(buffer, size, "Shift+F3");
      return buffer;
    case KEY_ARROW_UP:
      snprintf(buffer, size, "Up");
      return buffer;
    case KEY_ARROW_DOWN:
      snprintf(buffer, size, "Down");
      return buffer;
    case KEY_ARROW_LEFT:
      snprintf(buffer, size, "Left");
      return buffer;
    case KEY_ARROW_RIGHT:
      snprintf(buffer, size, "Right");
      return buffer;
    case KEY_HOME:
      snprintf(buffer, size, "Home");
      return buffer;
    case KEY_END:
      snprintf(buffer, size, "End");
      return buffer;
    case KEY_PAGE_UP:
      snprintf(buffer, size, "PgUp");
      return buffer;
    case KEY_PAGE_DOWN:
      snprintf(buffer, size, "PgDn");
      return buffer;
    case KEY_DELETE:
      snprintf(buffer, size, "Del");
      return buffer;
    case KEY_BACKSPACE:
      snprintf(buffer, size, "Backspace");
      return buffer;
    case KEY_SHIFT_TAB:
      snprintf(buffer, size, "Shift+Tab");
      return buffer;
    case KEY_CTRL_HOME:
      snprintf(buffer, size, "Ctrl+Home");
      return buffer;
    case KEY_CTRL_END:
      snprintf(buffer, size, "Ctrl+End");
      return buffer;
    case KEY_CTRL_ARROW_LEFT:
      snprintf(buffer, size, "Ctrl+Left");
      return buffer;
    case KEY_CTRL_ARROW_RIGHT:
      snprintf(buffer, size, "Ctrl+Right");
      return buffer;
    case KEY_SHIFT_ARROW_UP:
      snprintf(buffer, size, "Shift+Up");
      return buffer;
    case KEY_SHIFT_ARROW_DOWN:
      snprintf(buffer, size, "Shift+Down");
      return buffer;
    case KEY_SHIFT_ARROW_LEFT:
      snprintf(buffer, size, "Shift+Left");
      return buffer;
    case KEY_SHIFT_ARROW_RIGHT:
      snprintf(buffer, size, "Shift+Right");
      return buffer;
    case KEY_SHIFT_HOME:
      snprintf(buffer, size, "Shift+Home");
      return buffer;
    case KEY_SHIFT_END:
      snprintf(buffer, size, "Shift+End");
      return buffer;
    case KEY_SHIFT_PAGE_UP:
      snprintf(buffer, size, "Shift+PgUp");
      return buffer;
    case KEY_SHIFT_PAGE_DOWN:
      snprintf(buffer, size, "Shift+PgDn");
      return buffer;
    case KEY_CTRL_SHIFT_ARROW_LEFT:
      snprintf(buffer, size, "Ctrl+Shift+Left");
      return buffer;
    case KEY_CTRL_SHIFT_ARROW_RIGHT:
      snprintf(buffer, size, "Ctrl+Shift+Right");
      return buffer;
    case KEY_ALT_A:
      snprintf(buffer, size, "Alt+A");
      return buffer;
    case KEY_ALT_C:
      snprintf(buffer, size, "Alt+C");
      return buffer;
    case KEY_ALT_D:
      snprintf(buffer, size, "Alt+D");
      return buffer;
    case KEY_ALT_K:
      snprintf(buffer, size, "Alt+K");
      return buffer;
    case KEY_ALT_L:
      snprintf(buffer, size, "Alt+L");
      return buffer;
    case KEY_ALT_M:
      snprintf(buffer, size, "Alt+M");
      return buffer;
    case KEY_ALT_T:
      snprintf(buffer, size, "Alt+T");
      return buffer;
    case KEY_ALT_N:
      snprintf(buffer, size, "Alt+N");
      return buffer;
    case KEY_ALT_P:
      snprintf(buffer, size, "Alt+P");
      return buffer;
    case KEY_ALT_R:
      snprintf(buffer, size, "Alt+R");
      return buffer;
    case KEY_ALT_U:
      snprintf(buffer, size, "Alt+U");
      return buffer;
    case KEY_ALT_W:
      snprintf(buffer, size, "Alt+W");
      return buffer;
    case KEY_ALT_Z:
      snprintf(buffer, size, "Alt+Z");
      return buffer;
    case KEY_ALT_SHIFT_Z:
      snprintf(buffer, size, "Alt+Shift+Z");
      return buffer;
    case KEY_ALT_SHIFT_S:
      snprintf(buffer, size, "Alt+Shift+S");
      return buffer;
    case KEY_ALT_SHIFT_W:
      snprintf(buffer, size, "Alt+Shift+W");
      return buffer;
    case KEY_ALT_SHIFT_C:
      snprintf(buffer, size, "Alt+Shift+C");
      return buffer;
    case KEY_ALT_ARROW_UP:
      snprintf(buffer, size, "Alt+Up");
      return buffer;
    case KEY_ALT_ARROW_DOWN:
      snprintf(buffer, size, "Alt+Down");
      return buffer;
    case KEY_ALT_SLASH:
      snprintf(buffer, size, "Alt+/");
      return buffer;
    case KEY_ALT_BRACKET:
      snprintf(buffer, size, "Alt+]");
      return buffer;
    case KEY_CTRL_O:
      snprintf(buffer, size, "Ctrl+O");
      return buffer;
    case KEY_CTRL_N:
      snprintf(buffer, size, "Ctrl+N");
      return buffer;
    case KEY_CTRL_T:
      snprintf(buffer, size, "Ctrl+T");
      return buffer;
    case 27:
...
static char *safe_get_home(void) {
...
int keybinding_load_file(const char *path) {
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (*p == '[') {
      ...
      if (end) {
        *end = '\0';
        in_bindings_section = (strcasecmp(p + 1, "bindings") == 0);
      }
      continue;
    }
