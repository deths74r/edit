...
int output_buffer_append_checked(struct output_buffer *output, const char *text,
                                 size_t length) {
...
void output_buffer_append(struct output_buffer *output, const char *text,
                          size_t length) {
  int ret = output_buffer_append_checked(output, text, length);
  BUG_ON(ret);
}

void output_buffer_append_string(struct output_buffer *output,
                                 const char *text) {
...
uint32_t line_find_wrap_point(struct line *line, struct buffer *buffer,
                              uint32_t start_col, uint32_t max_width,
                              enum wrap_mode mode) {
  if (mode == WRAP_NONE) {
  ...
  while (col < line->cell_count) {
    ...
    if (cp == '\t') {
      ...
      for (uint32_t j = col; j < grapheme_end && j < line->cell_count; j++) {
        width = utf8_cpwidth(line->cells[j].codepoint);
        if (width > 0)
          break;
      }
      if (width <= 0)
  ...
  if (mode == WRAP_CHAR) {
    return (hard_break > start_col)
               ? hard_break
               : cursor_next_grapheme(line, buffer, start_col);
  }
...
void line_compute_wrap_points(struct line *line, struct buffer *buffer,
   ...
   */
  if (line->wrap_cache_width == text_width && line->wrap_cache_mode == mode &&
  ...
  while (column < line->cell_count) {
    uint32_t wrap_point =
        line_find_wrap_point(line, buffer, column, text_width, mode);
...
uint32_t editor_get_render_column(uint32_t row, uint32_t column) {
   ...
   */
  bool hybrid_active =
      editor.hybrid_mode && syntax_is_markdown_file(editor.buffer.filename);
