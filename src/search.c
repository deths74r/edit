...
/* Async search state */
static struct async_search_state async_search = {.current_match_index = -1};
...
int search_results_add_match(uint32_t row, uint32_t start_col,
                             uint32_t end_col) {
  pthread_mutex_lock(&async_search.results_mutex);
  ...
  if (async_search.results.match_count >= async_search.results.match_capacity) {
    uint32_t new_capacity = async_search.results.match_capacity == 0
                                ? 256
                                : async_search.results.match_capacity * 2;
    if (new_capacity > MAX_SEARCH_MATCHES) {
      new_capacity = MAX_SEARCH_MATCHES;
    }

    struct search_match *new_matches =
        realloc(async_search.results.matches,
  ...
  struct search_match *match =
      &async_search.results.matches[async_search.results.match_count++];
  match->row = row;
...
void search_results_update_progress(uint32_t rows_searched,
                                    uint32_t total_rows) {
  pthread_mutex_lock(&async_search.results_mutex);
...
bool search_should_use_async(void) {
  return worker_is_initialized() && async_search.mutex_initialized &&
...
uint32_t search_async_get_progress(bool *complete, uint32_t *rows_searched,
  ...
  if (complete)
    *complete = async_search.results.complete;
  if (rows_searched)
    *rows_searched = async_search.results.rows_searched;
  if (total_rows)
    *total_rows = async_search.results.total_rows;
  pthread_mutex_unlock(&async_search.results_mutex);
  return count;
}
...
void search_async_start(const char *pattern, bool use_regex,
  ...
  struct task task = {.type = TASK_SEARCH,
  ...
  if (err == 0) {
    ...
    log_debug("Started async search for '%s' (task %lu)", pattern,
              task.task_id);
  } else {
 ...
 */
bool search_async_is_active(void) { return async_search.active; }

/*
 * Get async search task ID for result matching.
 */
uint64_t search_async_get_task_id(void) { return async_search.task_id; }

/*
 * Mark async search as inactive (called when result received).
 */
void search_async_set_inactive(void) { async_search.active = false; }
...
int replace_results_add(uint32_t row, uint32_t start_col, uint32_t end_col,
  ...
  if (async_replace.results.count >= async_replace.results.capacity) {
    uint32_t new_capacity = async_replace.results.capacity == 0
                                ? 256
                                : async_replace.results.capacity * 2;

    struct replacement *new_array =
        realloc(async_replace.results.replacements,
  ...
  struct replacement *r =
      &async_replace.results.replacements[async_replace.results.count++];
  r->row = row;
...
void replace_results_update_progress(uint32_t rows_searched,
                                     uint32_t total_rows) {
  pthread_mutex_lock(&async_replace.results_mutex);
...
bool search_should_use_async_replace(void) {
  return worker_is_initialized() && async_replace.mutex_initialized &&
         editor.buffer.line_count > ASYNC_SEARCH_THRESHOLD;
}
...
uint32_t search_async_replace_get_progress(bool *search_complete,
  ...
  if (search_complete)
    *search_complete = async_replace.results.search_complete;
  if (apply_complete)
    *apply_complete = async_replace.results.apply_complete;
  if (total)
    *total = async_replace.results.total_rows;
  pthread_mutex_unlock(&async_replace.results_mutex);
  return count;
}
...
void search_async_replace_apply(void) {
  ...
  struct replacement *local_replacements =
      malloc(count * sizeof(struct replacement));
  ...
  for (int32_t i = count - 1; i >= 0; i--) {
    ...
    if (r->end_col > r->start_col) {
      buffer_delete_range_no_record(&editor.buffer, r->row, r->start_col,
                                    r->row, r->end_col);
    }
    ...
    if (r->replacement_len > 0) {
      ...
      while (byte_idx < r->replacement_len) {
        uint32_t codepoint;
        int bytes = utf8_decode(r->replacement_text + byte_idx,
                                r->replacement_len - byte_idx, &codepoint);
        if (bytes <= 0) {
          codepoint = 0xFFFD;
          bytes = 1;
        }
  ...
  editor_set_status_message("Replaced %u occurrence%s", applied,
                            applied == 1 ? "" : "s");
...
void search_async_replace_start(const char *pattern, const char *replacement,
  ...
  strncpy(async_replace.replacement, replacement,
          sizeof(async_replace.replacement) - 1);
  ...
  /* Submit task */
  struct task task = {.type = TASK_REPLACE_ALL,
                      .task_id = task_generate_id(),
                      .replace = {.use_regex = use_regex,
                                  .case_sensitive = case_sensitive,
                                  .whole_word = whole_word}};
  strncpy(task.replace.pattern, pattern, sizeof(task.replace.pattern) - 1);
  task.replace.pattern[sizeof(task.replace.pattern) - 1] = '\0';
  strncpy(task.replace.replacement, replacement,
          sizeof(task.replace.replacement) - 1);
  ...
  if (err == 0) {
    ...
    log_debug("Started async replace for '%s' -> '%s' (task %lu)", pattern,
              replacement, task.task_id);
  } else {
    log_warn("Failed to start async replace: %s", edit_strerror(err));
    editor_set_status_message("Failed to start replace: %s",
                              edit_strerror(err));
  }
}
 ...
 */
bool search_async_replace_is_active(void) { return async_replace.active; }
 ...
 */
void search_async_replace_set_inactive(void) { async_replace.active = false; }
