...
static uint8_t color_adjust_channel(uint8_t value, bool make_lighter) {
  if (make_lighter) {
    ...
    if (new_val > 255)
      new_val = 255;
    if (new_val == value && value < 255)
      new_val = value + 1;
    return (uint8_t)new_val;
  } else {
    /* Move toward 0 */
    int new_val = value / 2;
    if (new_val == value && value > 0)
      new_val = value - 1;
    return (uint8_t)new_val;
  }
}
 ...
 */
int style_to_escape(const struct style *style, char *buffer,
                    size_t buffer_size) {
  int length = 0;
...
int style_to_escape_with_bg(const struct style *style,
                            struct syntax_color bg_override, char *buffer,
                            size_t buffer_size) {
  int length = 0;
...
struct theme theme_create_default(void) {
  ...
  t.line_number = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.line_number_active = (struct style){
      .fg = {0x80, 0x80, 0x80}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_BOLD};

  /* Gutter */
  t.gutter = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.gutter_active = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_NONE};

  /* Status bar */
  t.status = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x2A, 0x2A, 0x2A}, .attr = ATTR_NONE};
  t.status_filename = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x2A, 0x2A, 0x2A}, .attr = ATTR_BOLD};
  t.status_modified = (struct style){
      .fg = {0xE0, 0xA0, 0x00}, .bg = {0x2A, 0x2A, 0x2A}, .attr = ATTR_BOLD};
  t.status_position = (struct style){
      .fg = {0xA0, 0xA0, 0xA0}, .bg = {0x2A, 0x2A, 0x2A}, .attr = ATTR_NONE};

  /* Message bar */
  t.message = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};

  /* Prompt components */
  t.prompt_label = (struct style){
      .fg = {0xA0, 0xA0, 0xD0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};
  t.prompt_input = (struct style){
      .fg = {0xFF, 0xFF, 0xFF}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.prompt_bracket = (struct style){
      .fg = {0x80, 0x80, 0xFF}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};
  t.prompt_warning = (struct style){
      .fg = {0xFF, 0xA0, 0x00}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};

  /* Search feedback */
  t.search_options = (struct style){
      .fg = {0x80, 0xC0, 0x80}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_DIM};
  t.search_nomatch = (struct style){
      .fg = {0xFF, 0x60, 0x60}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_ITALIC};
  t.search_error = (struct style){
      .fg = {0xFF, 0x40, 0x40}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};

  /* Whitespace */
  t.whitespace = (struct style){
      .fg = {0x38, 0x38, 0x38}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  ...
  t.wrap_indicator = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_DIM};
  t.empty_line = (struct style){
      .fg = {0x38, 0x38, 0x38}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_DIM};

  /* Bracket matching */
  t.bracket_match = (struct style){
      .fg = {0xFF, 0xFF, 0x00}, .bg = {0x50, 0x50, 0x00}, .attr = ATTR_BOLD};

  /* Multi-cursor */
  t.multicursor = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x40, 0x60, 0x80}, .attr = ATTR_NONE};

  /* Dialog */
  t.dialog = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_NONE};
  t.dialog_header = (struct style){
      .fg = {0xD0, 0xD0, 0xD0}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_BOLD};
  t.dialog_footer = (struct style){
      .fg = {0x80, 0x80, 0x80}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_DIM};
  t.dialog_highlight = (struct style){
      .fg = {0xFF, 0xFF, 0xFF}, .bg = {0x40, 0x40, 0x40}, .attr = ATTR_BOLD};
  t.dialog_directory = (struct style){
      .fg = {0x80, 0xB0, 0xFF}, .bg = {0x1A, 0x1A, 0x1A}, .attr = ATTR_BOLD};
  ...
  t.syntax[SYNTAX_KEYWORD] = (struct style){
      .fg = {0xFF, 0xFF, 0xFF}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};
  t.syntax[SYNTAX_TYPE] = (struct style){
      .fg = {0xE0, 0xE0, 0xE0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_STRING] = (struct style){
      .fg = {0xA0, 0xA0, 0xA0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_NUMBER] = (struct style){
      .fg = {0xC0, 0xC0, 0xC0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_COMMENT] = (struct style){
      .fg = {0x60, 0x60, 0x60}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_PREPROCESSOR] = (struct style){
      .fg = {0x90, 0x90, 0x90}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_FUNCTION] = (struct style){
      .fg = {0xF0, 0xF0, 0xF0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_OPERATOR] = (struct style){
      .fg = {0xB0, 0xB0, 0xB0}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_BRACKET] = (struct style){
      .fg = {0xC8, 0xC8, 0xC8}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_ESCAPE] = (struct style){
      .fg = {0xCC, 0xCC, 0xCC}, .bg = {0x0A, 0x0A, 0x0A}, .attr = ATTR_BOLD};
  ...
  t.syntax[SYNTAX_MD_HEADER_1] =
      (struct style){.fg = {0x56, 0xB4, 0xE9}, /* Sky blue - most prominent */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD | ATTR_UNDERLINE};
  t.syntax[SYNTAX_MD_HEADER_2] =
      (struct style){.fg = {0x56, 0xB4, 0xE9}, /* Sky blue */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_3] =
      (struct style){.fg = {0xE6, 0x9F, 0x00}, /* Orange */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_4] =
      (struct style){.fg = {0xE6, 0x9F, 0x00}, /* Orange - no bold */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_HEADER_5] =
      (struct style){.fg = {0x90, 0x90, 0x90}, /* Gray */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_6] =
      (struct style){.fg = {0x80, 0x80, 0x80}, /* Darker gray */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_NONE};
  ...
  t.syntax[SYNTAX_MD_ITALIC] = (struct style){
      ...
      .bg = {0x0A, 0x0A, 0x0A},
      .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_MD_BOLD_ITALIC] =
      (struct style){.fg = {0xF0, 0xE4, 0x42}, /* Yellow - high luminance */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD | ATTR_ITALIC};
  t.syntax[SYNTAX_MD_STRIKETHROUGH] =
      (struct style){.fg = {0x80, 0x80, 0x80}, /* Gray - dimmed appearance */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_CODE_SPAN] = (struct style){
      ...
      .bg = {0x1A, 0x1A, 0x1A},
      .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_CODE_BLOCK] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Bluish green */
                     .bg = {0x1A, 0x1A, 0x1A},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_CODE_FENCE_OPEN] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Same as code block */
                     .bg = {0x1A, 0x1A, 0x1A},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_CODE_FENCE_CLOSE] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Same as code block */
                     .bg = {0x1A, 0x1A, 0x1A},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_LINK_TEXT] =
      (struct style){.fg = {0x00, 0x72, 0xB2}, /* Blue - standard link color */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD | ATTR_UNDERLINE};
  t.syntax[SYNTAX_MD_LINK_URL] = (struct style){
      ...
      .bg = {0x0A, 0x0A, 0x0A},
      .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_IMAGE] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Bluish green */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD | ATTR_UNDERLINE};
  t.syntax[SYNTAX_MD_BLOCKQUOTE] =
      (struct style){.fg = {0xA0, 0xA0, 0xA0}, /* Gray - neutral */
                     .bg = {0x15, 0x15, 0x18},
                     .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_MD_LIST_MARKER] = (struct style){
      ...
      .bg = {0x0A, 0x0A, 0x0A},
      .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HORIZONTAL_RULE] =
      (struct style){.fg = {0xF0, 0xE4, 0x42}, /* Yellow */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_ESCAPE] =
      (struct style){.fg = {0xA0, 0xA0, 0xA0}, /* Gray */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_TABLE] =
      (struct style){.fg = {0x56, 0xB4, 0xE9}, /* Sky blue */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_TABLE_SEPARATOR] =
      (struct style){.fg = {0x60, 0x60, 0x60}, /* Dark gray - subtle */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_TABLE_HEADER] =
      (struct style){.fg = {0xE6, 0x9F, 0x00}, /* Orange - matches bold */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_TASK_MARKER] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Bluish green */
                     .bg = {0x0A, 0x0A, 0x0A},
                     .attr = ATTR_BOLD};
...
struct theme theme_create_mono_white(void) {
  ...
  /* Line numbers */
  t.line_number = (struct style){
      .fg = {0x90, 0x90, 0x90}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.line_number_active = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_BOLD};

  /* Gutter */
  t.gutter = (struct style){
      .fg = {0x90, 0x90, 0x90}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.gutter_active = (struct style){
      .fg = {0x90, 0x90, 0x90}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_NONE};

  /* Status bar */
  t.status = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xD8, 0xD8, 0xD8}, .attr = ATTR_NONE};
  t.status_filename = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xD8, 0xD8, 0xD8}, .attr = ATTR_BOLD};
  t.status_modified = (struct style){
      .fg = {0xA0, 0x60, 0x00}, .bg = {0xD8, 0xD8, 0xD8}, .attr = ATTR_BOLD};
  t.status_position = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0xD8, 0xD8, 0xD8}, .attr = ATTR_NONE};

  /* Message bar */
  t.message = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};

  /* Prompt components */
  t.prompt_label = (struct style){
      .fg = {0x40, 0x40, 0x80}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};
  t.prompt_input = (struct style){
      .fg = {0x00, 0x00, 0x00}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.prompt_bracket = (struct style){
      .fg = {0x40, 0x40, 0xA0}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};
  t.prompt_warning = (struct style){
      .fg = {0xC0, 0x60, 0x00}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};

  /* Search feedback */
  t.search_options = (struct style){
      .fg = {0x40, 0x80, 0x40}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_DIM};
  t.search_nomatch = (struct style){
      .fg = {0xC0, 0x30, 0x30}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_ITALIC};
  t.search_error = (struct style){
      .fg = {0xC0, 0x20, 0x20}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};

  /* Whitespace */
  t.whitespace = (struct style){
      .fg = {0xC0, 0xC0, 0xC0}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.whitespace_tab = (struct style){
      .fg = {0xC0, 0xC0, 0xC0}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.whitespace_space = (struct style){
      .fg = {0xC0, 0xC0, 0xC0}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  ...
  /* Wrap and special lines */
  t.wrap_indicator = (struct style){
      .fg = {0x90, 0x90, 0x90}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_DIM};
  t.empty_line = (struct style){
      .fg = {0xC0, 0xC0, 0xC0}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_DIM};

  /* Bracket matching */
  t.bracket_match = (struct style){
      .fg = {0x00, 0x00, 0x00}, .bg = {0xE0, 0xE0, 0x80}, .attr = ATTR_BOLD};

  /* Multi-cursor */
  t.multicursor = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xA0, 0xC0, 0xE0}, .attr = ATTR_NONE};

  /* Dialog */
  t.dialog = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_NONE};
  t.dialog_header = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_BOLD};
  t.dialog_footer = (struct style){
      .fg = {0x60, 0x60, 0x60}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_DIM};
  t.dialog_highlight = (struct style){
      .fg = {0x00, 0x00, 0x00}, .bg = {0xC8, 0xC8, 0xC8}, .attr = ATTR_BOLD};
  t.dialog_directory = (struct style){
      .fg = {0x30, 0x60, 0xA0}, .bg = {0xEC, 0xEC, 0xEC}, .attr = ATTR_BOLD};

  /* Syntax highlighting - grayscale with varying intensity */
  t.syntax[SYNTAX_NORMAL] = (struct style){
      .fg = {0x20, 0x20, 0x20}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_KEYWORD] = (struct style){
      .fg = {0x00, 0x00, 0x00}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};
  t.syntax[SYNTAX_TYPE] = (struct style){
      .fg = {0x18, 0x18, 0x18}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_STRING] = (struct style){
      .fg = {0x50, 0x50, 0x50}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_NUMBER] = (struct style){
      .fg = {0x38, 0x38, 0x38}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_COMMENT] = (struct style){
      .fg = {0x78, 0x78, 0x78}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_PREPROCESSOR] = (struct style){
      .fg = {0x60, 0x60, 0x60}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_FUNCTION] = (struct style){
      .fg = {0x10, 0x10, 0x10}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_OPERATOR] = (struct style){
      .fg = {0x40, 0x40, 0x40}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_BRACKET] = (struct style){
      .fg = {0x28, 0x28, 0x28}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_NONE};
  t.syntax[SYNTAX_ESCAPE] = (struct style){
      .fg = {0x30, 0x30, 0x30}, .bg = {0xF8, 0xF8, 0xF8}, .attr = ATTR_BOLD};
  ...
  t.syntax[SYNTAX_MD_HEADER_1] =
      (struct style){.fg = {0x00, 0x72, 0xB2}, /* Dark blue - most prominent */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD | ATTR_UNDERLINE};
  t.syntax[SYNTAX_MD_HEADER_2] =
      (struct style){.fg = {0x00, 0x72, 0xB2}, /* Dark blue */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_3] =
      (struct style){.fg = {0xD5, 0x5E, 0x00}, /* Vermilion */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_4] =
      (struct style){.fg = {0xD5, 0x5E, 0x00}, /* Vermilion - no bold */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_HEADER_5] =
      (struct style){.fg = {0x60, 0x60, 0x60}, /* Gray */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HEADER_6] =
      (struct style){.fg = {0x70, 0x70, 0x70}, /* Lighter gray */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_BOLD] = (struct style){
      ...
      .bg = {0xF8, 0xF8, 0xF8},
      .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_ITALIC] =
      (struct style){.fg = {0x88, 0x56, 0x78}, /* Muted reddish purple */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_MD_BOLD_ITALIC] =
      (struct style){.fg = {0x94, 0x40, 0x60}, /* Darker reddish purple */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD | ATTR_ITALIC};
  t.syntax[SYNTAX_MD_STRIKETHROUGH] =
      (struct style){.fg = {0x70, 0x70, 0x70}, /* Gray - dimmed appearance */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_CODE_SPAN] =
      (struct style){.fg = {0x40, 0x40, 0x40}, /* Dark gray */
  ...
  t.syntax[SYNTAX_MD_CODE_BLOCK] =
      (struct style){.fg = {0x40, 0x40, 0x40}, /* Dark gray */
                     ...
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_CODE_FENCE_OPEN] =
      (struct style){.fg = {0x40, 0x40, 0x40}, /* Same as code block */
                     .bg = {0xE8, 0xE8, 0xE8}, /* Light gray background */
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_CODE_FENCE_CLOSE] =
      (struct style){.fg = {0x40, 0x40, 0x40}, /* Same as code block */
                     ...
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_LINK_TEXT] =
      (struct style){.fg = {0x00, 0x72, 0xB2}, /* Dark blue - same as headers */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_UNDERLINE};
  t.syntax[SYNTAX_MD_LINK_URL] = (struct style){
  ...
  t.syntax[SYNTAX_MD_IMAGE] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Bluish green */
                     ...
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_BLOCKQUOTE] =
      (struct style){.fg = {0x60, 0x60, 0x70}, /* Dark gray with slight blue */
                     .bg = {0xF0, 0xF0, 0xF4}, /* Very light gray-blue */
                     .attr = ATTR_ITALIC};
  t.syntax[SYNTAX_MD_LIST_MARKER] =
      (struct style){.fg = {0xD5, 0x5E, 0x00}, /* Vermilion - same as bold */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_HORIZONTAL_RULE] =
      (struct style){.fg = {0x80, 0x80, 0x80}, /* Medium gray */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_ESCAPE] =
      (struct style){.fg = {0x50, 0x50, 0x50}, /* Dark gray */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_TABLE] =
      (struct style){.fg = {0x00, 0x72, 0xB2}, /* Dark blue */
                     ...
                     .attr = ATTR_NONE};
  t.syntax[SYNTAX_MD_TABLE_SEPARATOR] =
      (struct style){.fg = {0x90, 0x90, 0x90}, /* Light gray - subtle */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_DIM};
  t.syntax[SYNTAX_MD_TABLE_HEADER] =
      (struct style){.fg = {0xD5, 0x5E, 0x00}, /* Vermilion - matches bold */
                     .bg = {0xF8, 0xF8, 0xF8},
                     .attr = ATTR_BOLD};
  t.syntax[SYNTAX_MD_TASK_MARKER] =
      (struct style){.fg = {0x00, 0x9E, 0x73}, /* Bluish green */
  ...
  *t = theme_create_default();
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    *eq = '\0';
    ...
    } else if (strcmp(key, "search_current") == 0 &&
               color_parse_hex(value, &color)) {
      t->search_current = color;
    } else if (strcmp(key, "cursor_line") == 0 &&
               color_parse_hex(value, &color)) {
      t->cursor_line = color;
    } else if (strcmp(key, "color_column") == 0 &&
               color_parse_hex(value, &color)) {
      t->color_column = color;
    } else if (strcmp(key, "color_column_line") == 0 &&
               color_parse_hex(value, &color)) {
      t->color_column_line = color;
    } else if (strcmp(key, "trailing_ws") == 0 &&
               color_parse_hex(value, &color)) {
      t->trailing_ws = color;
    }
    ...
    } else if (strcmp(key, "line_number_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->line_number.fg = color;
    } else if (strcmp(key, "line_number_bg") == 0 &&
               color_parse_hex(value, &color)) {
      t->line_number.bg = color;
    } else if (strcmp(key, "line_number_attr") == 0) {
      t->line_number.attr = attr_parse(value);
    } else if (strcmp(key, "line_number_active") == 0 &&
               color_parse_hex(value, &color)) {
      t->line_number_active.fg = color; /* Legacy */
    } else if (strcmp(key, "line_number_active_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->line_number_active.fg = color;
    } else if (strcmp(key, "line_number_active_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    /* Gutter */
      ...
      t->gutter.fg = color;
    } else if (strcmp(key, "gutter_bg") == 0 &&
               color_parse_hex(value, &color)) {
      t->gutter.bg = color;
    } else if (strcmp(key, "gutter_attr") == 0) {
      t->gutter.attr = attr_parse(value);
    } else if (strcmp(key, "gutter_active_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->gutter_active.fg = color;
    } else if (strcmp(key, "gutter_active_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    /* Message bar */
      ...
      t->message.bg = color;
    } else if (strcmp(key, "message_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->message.fg = color;
    ...
    /* Wrap indicator */
    else if (strcmp(key, "wrap_indicator_fg") == 0 &&
             color_parse_hex(value, &color)) {
      t->wrap_indicator.fg = color;
    } else if (strcmp(key, "wrap_indicator_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    /* Empty line */
    else if (strcmp(key, "empty_line_fg") == 0 &&
             color_parse_hex(value, &color)) {
      t->empty_line.fg = color;
    } else if (strcmp(key, "empty_line_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    /* Bracket match */
    else if (strcmp(key, "bracket_match_fg") == 0 &&
             color_parse_hex(value, &color)) {
      t->bracket_match.fg = color;
    } else if (strcmp(key, "bracket_match_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    /* Multi-cursor */
    else if (strcmp(key, "multicursor_fg") == 0 &&
             color_parse_hex(value, &color)) {
      t->multicursor.fg = color;
    } else if (strcmp(key, "multicursor_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_normal") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_NORMAL].fg = color;
    } else if (strcmp(key, "syntax_keyword") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_KEYWORD].fg = color;
    } else if (strcmp(key, "syntax_type") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_TYPE].fg = color;
    } else if (strcmp(key, "syntax_string") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_STRING].fg = color;
    } else if (strcmp(key, "syntax_number") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_NUMBER].fg = color;
    } else if (strcmp(key, "syntax_comment") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_COMMENT].fg = color;
    } else if (strcmp(key, "syntax_preprocessor") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_PREPROCESSOR].fg = color;
    } else if (strcmp(key, "syntax_function") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_FUNCTION].fg = color;
    } else if (strcmp(key, "syntax_operator") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_OPERATOR].fg = color;
    } else if (strcmp(key, "syntax_bracket") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_BRACKET].fg = color;
    } else if (strcmp(key, "syntax_escape") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_ESCAPE].fg = color;
    }

    /* Syntax highlighting - new style fg/bg/attr */
    else if (strcmp(key, "syntax_normal_fg") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_NORMAL].fg = color;
    } else if (strcmp(key, "syntax_normal_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_keyword_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_KEYWORD].fg = color;
    } else if (strcmp(key, "syntax_keyword_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_type_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_TYPE].fg = color;
    } else if (strcmp(key, "syntax_type_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_string_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_STRING].fg = color;
    } else if (strcmp(key, "syntax_string_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_number_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_NUMBER].fg = color;
    } else if (strcmp(key, "syntax_number_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_comment_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_COMMENT].fg = color;
    } else if (strcmp(key, "syntax_comment_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_preprocessor_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_PREPROCESSOR].fg = color;
    } else if (strcmp(key, "syntax_preprocessor_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_function_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_FUNCTION].fg = color;
    } else if (strcmp(key, "syntax_function_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_operator_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_OPERATOR].fg = color;
    } else if (strcmp(key, "syntax_operator_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_bracket_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_BRACKET].fg = color;
    } else if (strcmp(key, "syntax_bracket_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_escape_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_ESCAPE].fg = color;
    } else if (strcmp(key, "syntax_escape_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_header_1") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_1].fg = color;
    } else if (strcmp(key, "syntax_md_header_1_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_1].fg = color;
    } else if (strcmp(key, "syntax_md_header_1_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_header_2") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_2].fg = color;
    } else if (strcmp(key, "syntax_md_header_2_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_2].fg = color;
    } else if (strcmp(key, "syntax_md_header_2_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_header_3") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_3].fg = color;
    } else if (strcmp(key, "syntax_md_header_3_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_3].fg = color;
    } else if (strcmp(key, "syntax_md_header_3_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_header_4") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_4].fg = color;
    } else if (strcmp(key, "syntax_md_header_4_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_4].fg = color;
    } else if (strcmp(key, "syntax_md_header_4_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_header_5") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_5].fg = color;
    } else if (strcmp(key, "syntax_md_header_5_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_5].fg = color;
    } else if (strcmp(key, "syntax_md_header_5_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_header_6") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_6].fg = color;
    } else if (strcmp(key, "syntax_md_header_6_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HEADER_6].fg = color;
    } else if (strcmp(key, "syntax_md_header_6_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_bold") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BOLD].fg = color;
    } else if (strcmp(key, "syntax_md_bold_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BOLD].fg = color;
    } else if (strcmp(key, "syntax_md_bold_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_italic") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_ITALIC].fg = color;
    } else if (strcmp(key, "syntax_md_italic_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_ITALIC].fg = color;
    } else if (strcmp(key, "syntax_md_italic_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_bold_italic") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BOLD_ITALIC].fg = color;
    } else if (strcmp(key, "syntax_md_bold_italic_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BOLD_ITALIC].fg = color;
    } else if (strcmp(key, "syntax_md_bold_italic_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_strikethrough") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_STRIKETHROUGH].fg = color;
    } else if (strcmp(key, "syntax_md_strikethrough_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_STRIKETHROUGH].fg = color;
    } else if (strcmp(key, "syntax_md_strikethrough_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_escape") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_ESCAPE].fg = color;
    } else if (strcmp(key, "syntax_md_escape_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_ESCAPE].fg = color;
    } else if (strcmp(key, "syntax_md_escape_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_code_span") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_SPAN].fg = color;
    } else if (strcmp(key, "syntax_md_code_span_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_SPAN].fg = color;
    } else if (strcmp(key, "syntax_md_code_span_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_code_block") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_BLOCK].fg = color;
    } else if (strcmp(key, "syntax_md_code_block_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_BLOCK].fg = color;
    } else if (strcmp(key, "syntax_md_code_block_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_code_fence_open_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_FENCE_OPEN].fg = color;
    } else if (strcmp(key, "syntax_md_code_fence_open_bg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_FENCE_OPEN].bg = color;
      t->syntax_bg_set[SYNTAX_MD_CODE_FENCE_OPEN] = true;
    } else if (strcmp(key, "syntax_md_code_fence_close_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_CODE_FENCE_CLOSE].fg = color;
    } else if (strcmp(key, "syntax_md_code_fence_close_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_link_text") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LINK_TEXT].fg = color;
    } else if (strcmp(key, "syntax_md_link_text_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LINK_TEXT].fg = color;
    } else if (strcmp(key, "syntax_md_link_text_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_link_url") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LINK_URL].fg = color;
    } else if (strcmp(key, "syntax_md_link_url_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LINK_URL].fg = color;
    } else if (strcmp(key, "syntax_md_link_url_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_image") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_IMAGE].fg = color;
    } else if (strcmp(key, "syntax_md_image_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_IMAGE].fg = color;
    } else if (strcmp(key, "syntax_md_image_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_blockquote") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BLOCKQUOTE].fg = color;
    } else if (strcmp(key, "syntax_md_blockquote_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_BLOCKQUOTE].fg = color;
    } else if (strcmp(key, "syntax_md_blockquote_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_list_marker") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LIST_MARKER].fg = color;
    } else if (strcmp(key, "syntax_md_list_marker_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_LIST_MARKER].fg = color;
    } else if (strcmp(key, "syntax_md_list_marker_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_horizontal_rule") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HORIZONTAL_RULE].fg = color;
    } else if (strcmp(key, "syntax_md_horizontal_rule_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_HORIZONTAL_RULE].fg = color;
    } else if (strcmp(key, "syntax_md_horizontal_rule_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_table") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE].fg = color;
    } else if (strcmp(key, "syntax_md_table_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE].fg = color;
    } else if (strcmp(key, "syntax_md_table_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_table_separator") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE_SEPARATOR].fg = color;
    } else if (strcmp(key, "syntax_md_table_separator_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE_SEPARATOR].fg = color;
    } else if (strcmp(key, "syntax_md_table_separator_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    } else if (strcmp(key, "syntax_md_table_header") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE_HEADER].fg = color;
    } else if (strcmp(key, "syntax_md_table_header_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TABLE_HEADER].fg = color;
    } else if (strcmp(key, "syntax_md_table_header_bg") == 0 &&
               color_parse_hex(value, &color)) {
    ...
    else if (strcmp(key, "syntax_md_task_marker") == 0 &&
             color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TASK_MARKER].fg = color;
    } else if (strcmp(key, "syntax_md_task_marker_fg") == 0 &&
               color_parse_hex(value, &color)) {
      t->syntax[SYNTAX_MD_TASK_MARKER].fg = color;
    } else if (strcmp(key, "syntax_md_task_marker_bg") == 0 &&
               color_parse_hex(value, &color)) {
...
void themes_load(void) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    ...
    struct theme *new_array =
        realloc(loaded_themes, (theme_count + 1) * sizeof(struct theme));
    if (new_array == NULL) {
      theme_free(parsed);
      free(parsed);
      continue;
  ...
  if (theme_count > 3) {
    qsort(&loaded_themes[2], theme_count - 2, sizeof(struct theme),
          theme_compare);
  }
}
...
/*
 * Helper to ensure style foreground has sufficient contrast against its
 * background.
 */
...
static void theme_apply(struct theme *t) {
  ...
  active_theme.foreground =
      color_ensure_contrast(t->foreground, active_theme.background);

  /* Color column line against background */
  active_theme.color_column_line =
      color_ensure_contrast(t->color_column_line, active_theme.background);
 ...
 */
struct theme *theme_get_active(void) { return &active_theme; }
 ...
 */
int theme_get_active_index(void) { return current_theme_index; }
