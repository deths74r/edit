 ...
 */
void undo_begin_group(struct buffer *buffer, uint32_t cursor_row,
                      uint32_t cursor_col) {
  ...
  if (history->group_count >= history->group_capacity) {
    uint32_t new_capacity = history->group_capacity == 0
                                ? INITIAL_UNDO_CAPACITY
                                : history->group_capacity * 2;
    struct undo_group *new_groups =
        realloc(history->groups, new_capacity * sizeof(struct undo_group));
    if (new_groups == NULL) {
 ...
 */
void undo_end_group(struct buffer *buffer, uint32_t cursor_row,
                    uint32_t cursor_col) {
  struct undo_history *history = &buffer->undo_history;
 ...
 */
static void undo_record_operation(struct buffer *buffer,
  ...
  if (group->operation_count >= group->operation_capacity) {
    uint32_t new_capacity = group->operation_capacity == 0
                                ...
                                : group->operation_capacity * 2;
    struct edit_operation *new_ops = realloc(
        group->operations, new_capacity * sizeof(struct edit_operation));
    if (new_ops == NULL) {
...
void undo_record_insert_char(struct buffer *buffer, uint32_t row,
  ...
  struct edit_operation op = {.type = EDIT_OP_INSERT_CHAR,
                              .row = row,
                              .column = column,
                              .codepoint = codepoint,
                              .text = NULL,
...
void undo_record_delete_char(struct buffer *buffer, uint32_t row,
                             uint32_t column, uint32_t codepoint) {
  struct edit_operation op = {.type = EDIT_OP_DELETE_CHAR,
                              .row = row,
                              .column = column,
                              .codepoint = codepoint,
                              .text = NULL,
...
void undo_record_insert_newline(struct buffer *buffer, uint32_t row,
                                uint32_t column) {
  struct edit_operation op = {.type = EDIT_OP_INSERT_NEWLINE,
                              .row = row,
                              .column = column,
                              .codepoint = 0,
                              .text = NULL,
...
void undo_record_delete_newline(struct buffer *buffer, uint32_t row,
                                uint32_t column) {
  struct edit_operation op = {.type = EDIT_OP_DELETE_NEWLINE,
                              .row = row,
                              .column = column,
                              .codepoint = 0,
                              .text = NULL,
...
void undo_record_delete_text(struct buffer *buffer, uint32_t start_row,
                             uint32_t start_col, uint32_t end_row,
                             uint32_t end_col, const char *text,
                             size_t text_length) {
  struct edit_operation op = {.type = EDIT_OP_DELETE_TEXT,
                              .row = start_row,
                              .column = start_col,
                              .codepoint = 0,
                              .text = NULL,
...
/*
 * Insert UTF-8 text at the specified position without recording to undo
 * history. Used during undo/redo operations.
 */
static void buffer_insert_text_no_record(struct buffer *buffer, uint32_t row,
  ...
  while (offset < text_length) {
    uint32_t codepoint;
    int bytes = utf8_decode(text + offset, text_length - offset, &codepoint);
 ...
 */
void buffer_delete_range_no_record(struct buffer *buffer, uint32_t start_row,
                                   uint32_t start_col, uint32_t end_row,
                                   uint32_t end_col) {
 ...
 */
static void undo_reverse_operation(struct buffer *buffer,
                                   struct edit_operation *op) {
  switch (op->type) {
  ...
  case EDIT_OP_DELETE_TEXT:
    ...
    buffer_insert_text_no_record(buffer, op->row, op->column, op->text,
                                 op->text_length);
    break;
  }
}
 ...
 */
static void undo_apply_operation(struct buffer *buffer,
                                 struct edit_operation *op) {
  switch (op->type) {
  case EDIT_OP_INSERT_CHAR:
  ...
  case EDIT_OP_DELETE_TEXT:
    /* Delete text range */
    buffer_delete_range_no_record(buffer, op->row, op->column, op->end_row,
                                  op->end_column);
    break;
  }
}
 ...
 */
bool undo_perform(struct buffer *buffer, uint32_t *cursor_row,
                  uint32_t *cursor_col) {
  struct undo_history *history = &buffer->undo_history;
 ...
 */
bool redo_perform(struct buffer *buffer, uint32_t *cursor_row,
                  uint32_t *cursor_col) {
  struct undo_history *history = &buffer->undo_history;
